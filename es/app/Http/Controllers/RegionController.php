...
use App\Http\Requests\region\RegionQueryRequest;
use App\Http\Requests\region\RegionUpdateRequest;
use App\Http\Responses\region\RegionQueryResponse;
use App\Http\Responses\region\RegionResponse;
...
{
    #[OA\Post(path: '/api/region/query', summary: '查询列表接口', security: [['bearerAuth' => []]], tags: ['行政地区模块'])]
    ...
    #[OA\Post(path: '/api/region/create', summary: '新增接口', security: [['bearerAuth' => []]], tags: ['行政地区模块'])]
    ...
    public function create(): Response
    {
        ...
        try {
            ...
            $v = new RegionCreateRequest();
            if (! $v->check($request)) {
                throw new CustomException($v->getError());
            }

            $input = new RegionEntity;
            $input->setData($request);

            $regionService = new RegionService();
            $insertId = $regionService->save($input->toArray());
            if ($insertId > 0) {
                DB::commit();
    ...
    #[OA\Get(path: '/api/region/show', summary: '获取详情接口', security: [['bearerAuth' => []]], tags: ['行政地区模块'])]
    ...
    public function show(): Response
    {
        try {
            ...
            $regionService = new RegionService();
            $region = $regionService->getOne($condition);
    ...
    #[OA\Put(path: '/api/region/update', summary: '更新接口', security: [['bearerAuth' => []]], tags: ['行政地区模块'])]
    ...
    public function update(): Response
    {
        ...
        try {
            ...
            $v = new RegionUpdateRequest();
            if (! $v->check($request)) {
                throw new CustomException($v->getError());
            }

            $regionService = new RegionService();
            $region = $regionService->getById($request['id']);
            if (empty($region)) {
                throw new CustomException('数据不存在或状态异常');
            }
            ...
            $regionService->update($input->toArray(), [
                ['id', '=', $request['id']],
            ]);
    ...
    #[OA\Delete(path: '/api/region/destroy', summary: '删除接口', security: [['bearerAuth' => []]], tags: ['行政地区模块'])]
    #[OA\Parameter(name: 'id', description: 'ID', in: 'query', required: true, example: 1)]
    #[OA\Response(response: 200, description: 'OK')]
    public function destroy(): Response
    {
        ...
        try {
            ...
            $regionService = new RegionService();
            if ($regionService->remove($condition)) {
                DB::commit();
