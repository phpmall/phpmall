...
class ArticleCatService extends ArticleCatBundleService
{
    /**
     * 获得指定分类下的子分类的数组
     *
    ...
    public function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        ...
        if ($res === null) {
            $data = Cache::get('art_cat_pid_releate');
            if ($data === null) {
                $res = DB::table('article_cat as c')
                    ->leftJoin('article_cat as s', 's.parent_id', '=', 'c.cat_id')
                    ->leftJoin('article as a', 'a.cat_id', '=', 'c.cat_id')
                    ->select('c.*', DB::raw('COUNT(s.cat_id) AS has_children'), DB::raw('COUNT(a.article_id) AS aricle_num'))
                    ->groupBy('c.cat_id')
                    ->orderBy('c.parent_id')
                    ...
                    ->all();
                Cache::forever('art_cat_pid_releate', $res);
            } else {
                $res = $data;
            }
        }
