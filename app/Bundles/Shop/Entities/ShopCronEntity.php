...
class ShopCronEntity
{
    ...
    const string getCronCode = 'cron_code'; // 计划任务代码

    const string getCronName = 'cron_name'; // 计划任务名称

    const string getCronDesc = 'cron_desc'; // 计划任务描述

    const string getCronOrder = 'cron_order'; // 排序

    const string getCronConfig = 'cron_config'; // 计划任务配置

    const string getThistime = 'thistime'; // 本次执行时间

    const string getNextime = 'nextime'; // 下次执行时间

    const string getDay = 'day'; // 日

    const string getWeek = 'week'; // 周

    const string getHour = 'hour'; // 时

    const string getMinute = 'minute'; // 分

    const string getEnable = 'enable'; // 是否启用

    const string getRunOnce = 'run_once'; // 是否只运行一次

    const string getAllowIp = 'allow_ip'; // 允许的IP

    const string getAlowFiles = 'alow_files'; // 允许的文件
