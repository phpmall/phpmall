<?php

declare(strict_types=1);

namespace App\Bundles\Email\Repositories;

use App\Bundles\Email\Entities\EmailTemplateEntity;
use App\Bundles\Email\Models\EmailTemplate;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Query\Builder;
use Illuminate\Support\Facades\DB;
use Juling\Foundation\Contracts\RepositoryInterface;
use Juling\Foundation\Repositories\CurdRepository;

class EmailTemplateRepository extends CurdRepository implements RepositoryInterface
{
    private static ?EmailTemplateRepository $instance = null;

    /**
     * 单例 EmailTemplateRepository
     */
    public static function getInstance(): EmailTemplateRepository
    {
        if (is_null(self::$instance)) {
            self::$instance = new EmailTemplateRepository;
        }

        return self::$instance;
    }

    /**
     * 添加 EmailTemplateEntity
     */
    public function saveEntity(EmailTemplateEntity $entity): int
    {
        return $this->save($entity->toEntity());
    }

    /**
     * 按照ID查询返回对象
     */
    public function findOneById(int $id): ?EmailTemplateEntity
    {
        $data = $this->findById($id);
        if (empty($data)) {
            return null;
        }

        return new EmailTemplateEntity($data);
    }

    /**
     * 按照条件查询返回对象
     */
    public function findOne(array $condition = []): ?EmailTemplateEntity
    {
        $data = $this->find($condition);
        if (empty($data)) {
            return null;
        }

        return new EmailTemplateEntity($data);
    }

    /**
     * 定义数据表查询构造器
     */
    public function builder(): Builder
    {
        return DB::table('email_template');
    }

    /**
     * 定义数据表模型类
     */
    public function model(): Model
    {
        return new EmailTemplate;
    }
}
