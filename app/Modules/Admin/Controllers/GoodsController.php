...
class GoodsController extends BaseController
{
    public function index(Request $request)
    {
        ...
        $exc = new Exchange(ecs()->table('goods'), null, 'goods_id', 'goods_name');
        ...
        if ($action === 'list' || $action === 'trash') {
            ...
            // 供货商名
            $suppliers_list_name = MainHelper::suppliers_list_name();
            $suppliers_exists = 1;
            ...
            // 获取商品类型存在规格的类型
            $specifications = GoodsHelper::get_goods_type_specifications();
        ...
        if ($action === 'add' || $action === 'edit' || $action === 'copy') {
            ...
            if ($is_add) {
                ...
                // 商品信息
                $goods = DB::table('goods')->where('goods_id', $_REQUEST['goods_id'])->first();
                ...
                $_attribute = GoodsHelper::get_goods_specifications_list($goods['goods_id']);
                ...
                if ($action === 'copy') {
                    ...
                    $res = DB::table('goods_attr')
                        ...
                        ->get();
                    foreach ($res as $row) {
                        DB::table('goods_attr')->insert((array) BaseHelper::addslashes_deep((array) $row));
                    }
                }

                // 扩展分类
                $goods['other_cat'] = DB::table('goods_cat')
                    ->where('goods_id', $_REQUEST['goods_id'])
                    ->pluck('cat_id')
                    ->toArray();
        ...
        if ($action === 'insert' || $action === 'update') {
            ...
            // 如果上传了商品图片，相应处理
            if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != lang('lab_picture_url') && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1)) {
            ...
            if ($is_insert) {
                ...
                if ($code !== '') {
                    $goods_data['is_real'] = 0;
                    $goods_data['extension_code'] = $code;
                }
                ...
                // 如果有上传图片，需要更新数据库
                if ($goods_img) {
                    $update_data['goods_img'] = $goods_img;
                    $update_data['original_img'] = $original_img;
                }
                if ($goods_thumb) {
                    $update_data['goods_thumb'] = $goods_thumb;
                }
                if ($code != '') {
                    $update_data['is_real'] = 0;
                    $update_data['extension_code'] = $code;
                }
            ...
            if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
                ...
                $attr_res = DB::table('goods_type_attribute')
                    ->select('attr_id', 'attr_index')
            ...
            // 处理相册图片
            GoodsHelper::handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file']);
            ...
            // 记录上一次选择的分类和品牌
            setcookie('ECSCP[last_choose]', $catgory_id.'|'.$brand_id, TimeHelper::gmtime() + 86400, '', '', false, true);
            // 清空缓存
            $this->clear_cache_files();

            // 提示页面
            $link = [];
            if (GoodsHelper::check_goods_specifications_exist($goods_id)) {
                $link[0] = ['href' => 'goods.php?act=product_list&goods_id='.$goods_id, 'text' => lang('product')];
            }
            if ($code === 'virtual_card') {
                $link[1] = ['href' => 'virtual_card.php?act=replenish&goods_id='.$goods_id, 'text' => lang('add_replenish')];
            }
            if ($is_insert) {
        ...
        if ($action === 'batch') {
            ...
            if (isset($_POST['type'])) {
                ...
                elseif ($_POST['type'] === 'suppliers_move_to') {
                    $this->admin_priv('goods_manage');
                    GoodsHelper::update_goods($goods_id, 'suppliers_id', $_POST['suppliers_id']);
                } // 还原
        ...
        if ($action === 'edit_goods_sn') {
            ...
            if (DB::table('goods_product')->where('product_sn', $goods_sn)->exists()) {
                return $this->make_json_error(lang('goods_sn_exists'));
            }
        ...
        if ($action === 'check_goods_sn') {
            ...
            $goods_sn = htmlspecialchars(BaseHelper::json_str_iconv(trim($_REQUEST['goods_sn'])));
        ...
        if ($action === 'check_products_goods_sn') {
            $this->check_authz_json('goods_manage');
                ...
                foreach ($products_sn as $val) {
                    ...
                    if (is_array($int_arry)) {
                        if (in_array($val, $int_arry)) {
                            return $this->make_json_error($val.lang('goods_sn_exists'));
                        }
                    }
                    $int_arry[] = $val;
                    ...
                    if (DB::table('goods_product')->where('product_sn', $val)->exists()) {
                        return $this->make_json_error($val.lang('goods_sn_exists'));
                    }
                }
            }
        ...
        if ($action === 'edit_goods_number') {
            ...
            if (GoodsHelper::check_goods_product_exist($goods_id) === 1) {
                return $this->make_json_error(lang('sys.wrong').lang('cannot_goods_number'));
            }
        ...
        if ($action === 'drop_goods') {
            ...
            // 删除商品的货品记录
            DB::table('goods_product')->where('goods_id', $goods_id)->delete();
        ...
        if ($action === 'drop_link_goods') {
            ...
            if (! $is_signle) {
                DB::table('goods_link_goods')
                    ->where('link_goods_id', $goods_id)
                    ->whereIn('goods_id', $drop_goods)
                    ->when($goods_id === 0, function ($query) {
                        return $query->where('admin_id', Session::get('admin_id'));
                    })
                    ->delete();
            } else {
                DB::table('goods_link_goods')
            ...
            DB::table('goods_link_goods')
                ->where('goods_id', $goods_id)
                ->whereIn('link_goods_id', $drop_goods)
                ->when($goods_id === 0, function ($query) {
                    return $query->where('admin_id', Session::get('admin_id'));
                })
                ->delete();
        ...
        if ($action === 'get_article_list') {
            ...
            $res = DB::table('article')
                ...
                ->when(! empty($filters['title']), function ($query) use ($filters) {
                    return $query->where('title', 'LIKE', '%'.BaseHelper::mysql_like_quote(trim($filters['title'])).'%');
                })
        ...
        if ($action === 'product_list') {
            ...
            if (empty($_GET['goods_id'])) {
                $link[] = ['href' => 'goods.php?act=list', 'text' => lang('cannot_found_goods')];

                return $this->sys_msg(lang('cannot_found_goods'), 1, $link);
            } else {
            ...
            // 取出商品信息
            $goods = DB::table('goods')
                ->select('goods_sn', 'goods_name', 'goods_type', 'shop_price')
                ->where('goods_id', $goods_id)
                ->first();
            $goods = $goods ? (array) $goods : [];
            if (empty($goods)) {
            ...
            $this->assign('sn', sprintf(lang('good_goods_sn'), $goods['goods_sn']));
            $this->assign('price', sprintf(lang('good_shop_price'), $goods['shop_price']));
            $this->assign('goods_name', sprintf(lang('products_title'), $goods['goods_name']));
            $this->assign('goods_sn', sprintf(lang('products_title_2'), $goods['goods_sn']));

            // 获取商品规格列表
            $attribute = GoodsHelper::get_goods_specifications_list($goods_id);
            if (empty($attribute)) {
                $link[] = ['href' => 'goods.php?act=edit&goods_id='.$goods_id, 'text' => lang('edit_goods')];

                return $this->sys_msg(lang('not_exist_goods_attr'), 1, $link);
            }
            ...
            $this->assign('product_number', cfg('default_storage'));

            // 取商品的货品
            $product = GoodsHelper::product_list($goods_id, '');
            ...
            $this->assign('ur_here', lang('18_product_list'));
            $this->assign('action_link', ['href' => 'goods.php?act=list', 'text' => lang('01_goods_list')]);
            ...
            return $this->display('product_info');
        }
        ...
        if ($action === 'product_query') {
            ...
            if (empty($_REQUEST['goods_id'])) {
                return $this->make_json_error(lang('sys.wrong').lang('cannot_found_goods'));
            } else {
                $goods_id = intval($_REQUEST['goods_id']);
            }
            ...
            // 排序标记
            $sort_flag = MainHelper::sort_flag($product['filter']);
            $this->assign($sort_flag['tag'], $sort_flag['img']);

            return $this->make_json_result(
                $this->fetch('product_info'),
                '',
        ...
        if ($action === 'product_remove') {
            ...
            if (empty($_REQUEST['id'])) {
                return $this->make_json_error(lang('product_id_null'));
            } else {
                $product_id = intval($_REQUEST['id']);
            }

            // 货品库存
            $product = GoodsHelper::get_product_info($product_id, 'product_number, goods_id');

            // 删除货品
            $result = DB::table('goods_product')->where('product_id', $product_id)->delete();
            if ($result) {
                // 修改商品库存
                if (update_goods_stock($product['goods_id'], $product_number - $product['product_number'])) {
                    // 记录日志
                    $this->admin_log('', 'update', 'goods');
                }

                // 记录日志
                $this->admin_log('', 'trash', 'products');
         ...
         * 修改货品价格
         */
        if ($action === 'edit_product_sn') {
            $this->check_authz_json('goods_manage');

            $product_id = intval($_POST['id']);
            $product_sn = BaseHelper::json_str_iconv(trim($_POST['val']));
            $product_sn = (lang('n_a') === $product_sn) ? '' : $product_sn;

            if (GoodsHelper::check_product_sn_exist($product_sn, $product_id)) {
                return $this->make_json_error(lang('sys.wrong').lang('exist_same_product_sn'));
            }

            // 修改
            $result = DB::table('goods_product')
                ->where('product_id', $product_id)
                ->update(['product_sn' => $product_sn]);
            if ($result) {
                $this->clear_cache_files();

                return $this->make_json_result($product_sn);
            }
        }
        ...
        if ($action === 'edit_product_number') {
            ...
            if ($result) {
                ...
                if (update_goods_stock($product['goods_id'], $product_number - $product['product_number'])) {
                    ...
                    return $this->make_json_result($product_number);
                }
            }
        }
        ...
        if ($action === 'product_add_execute') {
            ...
            if (empty($product['goods_id'])) {
                return $this->sys_msg(lang('sys.wrong').lang('cannot_found_goods'), 1, [], false);
            }

            // 判断是否为初次添加
            $insert = true;
            if (GoodsHelper::product_number_count($product['goods_id']) > 0) {
                $insert = false;
            }
            ...
            foreach ($product['product_sn'] as $key => $value) {
                // 过滤
                $product['product_number'][$key] = empty($product['product_number'][$key]) ? (empty(cfg('use_storage')) ? 0 : cfg('default_storage')) : trim($product['product_number'][$key]); // 库存
                ...
                $goods_attr_id = GoodsHelper::handle_goods_attr($product['goods_id'], $id_list, $is_spec_list, $value_price_list);

                // 是否为重复规格的货品
                $goods_attr = CommonHelper::sort_goods_attr_id_array($goods_attr_id);
                $goods_attr = implode('|', $goods_attr['sort']);
                if (GoodsHelper::check_goods_attr_exist($goods_attr, $product['goods_id'])) {
                    continue;
                    // return $this->sys_msg(lang('sys['wrong']wrong') . lang('exist_same_goods_attr'), 1, array(), false);
                }
                // 货品号不为空
                if (! empty($value)) {
                    // 检测：货品货号是否在商品表和货品表中重复
                    if (GoodsHelper::check_goods_sn_exist($value)) {
                        continue;
                        // return $this->sys_msg(lang('sys['wrong']wrong') . lang('exist_same_goods_sn'), 1, array(), false);
                    }
                    if (GoodsHelper::check_product_sn_exist($value)) {
                        continue;
                        // return $this->sys_msg(lang('sys['wrong']wrong') . lang('exist_same_product_sn'), 1, array(), false);
                    }
                }

                // 插入货品表
                $product_id = DB::table('goods_product')->insertGetId([
                ...
                if (empty($value)) {
                    DB::table('goods_product')
                        ->where('product_id', $product_id)
                        ->update(['product_sn' => $goods['goods_sn'].'g_p'.$product_id]);
                }

                // 修改商品表库存
                $product_count = GoodsHelper::product_number_count($product['goods_id']);
                if (GoodsHelper::update_goods($product['goods_id'], 'goods_number', $product_count)) {
                    // 记录日志
                    $this->admin_log($product['goods_id'], 'update', 'goods');
                }
            }
            ...
            // 返回
            if ($insert) {
                $link[] = ['href' => 'goods.php?act=add', 'text' => lang('02_goods_add')];
                $link[] = ['href' => 'goods.php?act=list', 'text' => lang('01_goods_list')];
                $link[] = ['href' => 'goods.php?act=product_list&goods_id='.$product['goods_id'], 'text' => lang('18_product_list')];
            } else {
                $link[] = ['href' => 'goods.php?act=list&uselastfilter=1', 'text' => lang('01_goods_list')];
                $link[] = ['href' => 'goods.php?act=edit&goods_id='.$product['goods_id'], 'text' => lang('edit_goods')];
                $link[] = ['href' => 'goods.php?act=product_list&goods_id='.$product['goods_id'], 'text' => lang('18_product_list')];
            }

            return $this->sys_msg(lang('save_products'), 0, $link);
        }

        /**
         * 货品批量操作
         */
        if ($action === 'batch_product') {
            // 定义返回
            $link[] = ['href' => 'goods.php?act=product_list&goods_id='.$_POST['goods_id'], 'text' => lang('item_list')];
            ...
            if ($_POST['type'] === 'drop') {
                ...
                $sum = 0;
                $goods_id = 0;
                $product_array = DB::table('goods_product')
                    ->select('product_id', 'goods_id', 'product_number')
                    ->whereIn('product_id', $_POST['checkboxes'])
                    ->get()
                    ->toArray();
                if (! empty($product_array)) {
                    ...
                    // 删除货品
                    if (DB::table('goods_product')->whereIn('product_id', $_POST['checkboxes'])->delete()) {
                        // 记录日志
                        $this->admin_log('', 'delete', 'products');
                    }
                    ...
                    // 返回
                    return $this->sys_msg(lang('product_batch_del_success'), 0, $link);
                } else {
                    // 错误
                    return $this->sys_msg(lang('cannot_found_products'), 1, $link);
                }
            }

            // 返回
            return $this->sys_msg(lang('no_operation'), 1, $link);
        }
    }
    ...
    private function update_goods_stock($goods_id, $value)
    {
        if ($goods_id) {
            ...
            $result = DB::table('goods')
                ->where('goods_id', $goods_id)
                ->update([
                    'goods_number' => DB::raw("goods_number + $value"),
                    'last_update' => TimeHelper::gmtime(),
                ]);
