        ...
        <fieldset style="border:1px solid #DDEEF2">
            <table>
                <tr>
                    ...
                    <td><input type="text" name="shipping_area_name" maxlength="60" size="30"
                               value="{{ $shipping_area['shipping_area_name'] }}"/>{{ $lang['require_field'] }}</td>
                </tr>
                @if($shipping_area['shipping_code'] =='ems' || $shipping_area['shipping_code'] =='yto' || $shipping_area['shipping_code'] =='zto' || $shipping_area['shipping_code'] =='sto_express' || $shipping_area['shipping_code'] =='post_mail' || $shipping_area['shipping_code'] =='sf_express' || $shipping_area['shipping_code'] =='post_express' )
                    <tr>
                        ...
                        <td>
                            <input type="radio" @if($fee_compute_mode != 'by_number' )checked="true"
                                   @endif onclick="compute_mode('{{ $shipping_area['shipping_code'] }}','weight')"
                                   name="fee_compute_mode" value="by_weight"/>{{ $lang['fee_by_weight'] }}
                            <input type="radio" @if($fee_compute_mode === 'by_number')checked="true"
                                   @endif  onclick="compute_mode('{{ $shipping_area['shipping_code'] }}','number')"
                                   name="fee_compute_mode" value="by_number"/>{{ $lang['fee_by_number'] }}
                        </td>
                    </tr>
                @endif
                        ...
                        @if($fee_compute_mode === 'by_number')
                            @if($field['name'] === 'item_fee' || $field['name'] === 'free_money' || $field['name'] === 'pay_fee')
                                <tr id="{{ $field['name'] }}">
                                    ...
                                    <td><input type="text" name="{{ $field['name'] }}" maxlength="60" size="20"
                                               value="{{ $field['value'] }}"/>{{ $lang['require_field'] }}</td>
                                </tr>
                            @else
            ...
            <table style="width:600px" align="center">
                ...
                <tr>
                    <td>
                        ...
                        <span style="vertical-align: top"><input type="button" value="+" class="button"
                                                                 onclick="addRegion()"/></span>
                    </td>
                </tr>
            </table>
        </fieldset>
