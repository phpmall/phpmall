    ...
    <form method="post" action="articlecat.php" name="theForm" onsubmit="return validate()">
        <table cellspacing="1" cellpadding="3" width="100%">
            <tr>
                ...
                <td><input type="text" name="cat_name" maxlength="60" size="30"
                           value="{{ $cat['cat_name'] }}"/>{{ $lang['require_field'] }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name='sort_order' @if($cat['sort_order']) value='{{ $cat['sort_order'] }}' @else
                        value="50" @endif size="15"/>
                </td>
            </tr>
            <tr>
                <td class="label">{{ $lang['show_in_nav'] }}:</td>
                <td>
                    <input type="radio" name="show_in_nav" value="1" @if($cat['show_in_nav'] != 0) checked="true"
                        @endif />
                    {{ $lang['yes'] }}
                    <input type="radio" name="show_in_nav" value="0" @if($cat['show_in_nav'] === 0) checked="true"
                        @endif /> {{ $lang['no'] }}
                </td>
            </tr>
            <tr>
                <td class="label"><a href="javascript:showNotice('notice_keywords');"
                                     title="{{ $lang['form_notice'] }}">
                        <img src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                             alt="{{ $lang['form_notice'] }}"></a>{{ $lang['cat_keywords'] }}</td>
                <td><input type="text" name="keywords" maxlength="60" size="50" value="{{ $cat['keywords'] }}"/>
                    <br/><span class="notice-span" @if($help_open) style="display:block" @else style="display:none"
                               @endif id="notice_keywords">{{ $lang['notice_keywords'] }}</span>
                </td>
            </tr>
            <tr>
