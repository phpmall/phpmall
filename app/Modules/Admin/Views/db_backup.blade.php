...
<form name="theForm" method="post" action="database.php" onsubmit="return validate()">
    ...
    <div class="list-div" id="listDiv">
        ...
        <table cellspacing='1' cellpadding='3'>
            ...
            <tr>
                <td><input type="radio" name="type" value="full" class="radio"
                           onclick="findobj('showtables').style.display='none'">{{ $lang['full_backup'] }}</td>
                <td>{{ $lang['full_backup_note'] }}</td>
            </tr>
            <tr>
                <td><input type="radio" name="type" value="stand" class="radio" checked="checked"
                           onclick="findobj('showtables').style.display='none'">{{ $lang['stand_backup'] }}</td>
                <td>{{ $lang['stand_backup_note'] }}</td>
            </tr>
            <tr>
                <td><input type="radio" name="type" value="min" class="radio"
                           onclick="findobj('showtables').style.display='none'">{{ $lang['min_backup'] }}</td>
                <td>{{ $lang['min_backup_note'] }}</td>
            </tr>
            <tr>
                <td><input type="radio" name="type" value="custom" class="radio"
                           onclick="findobj('showtables').style.display=''">{{ $lang['custom_backup'] }}</td>
            ...
            <tr>
                <td colspan="2">
                    <table>
                        <tr>
                            <td colspan="4"><input name="chkall" onclick="checkall(this.form, 'customtables[]')"
                                                   type="checkbox"><b>{{ $lang['check_all'] }}</b></td>
                        </tr>
                        <tr>
                            @foreach($tables as $table)
                                @if($loop->table_name . iteration > 1 && ($loop->table_name . iteration - 1) % 4 === 0)
                        </tr>
                        <tr>
                            @endif
        ...
        <table cellspacing='1' cellpadding='3'>
            ...
            <tr>
                ...
                <td><input type="radio" name="ext_insert" class="radio"
                           value='1'>{{ $lang['yes'] }}&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="ext_insert"
                                                                                      class="radio" value='0'
                                                                                      checked="checked">{{ $lang['no'] }}
                </td>
            </tr>
            <tr>
                <td>{{ $lang['vol_size'] }}</td>
