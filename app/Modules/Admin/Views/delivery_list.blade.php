    ...
    <form method="post" action="order.php?act=operate" name="listForm" onsubmit="return check()">
        <div class="list-div" id="listDiv">
            ...
            <table cellpadding="3" cellspacing="1">
                <tr>
                    <th>
                        <input onclick='listTable.selectAll(this, "delivery_id")' type="checkbox"/><a
                    ...
                    <th>{{ $lang['handler'] }}</th>
                <tr>
                @foreach($delivery_list as $delivery => $dkey)
                    <tr>
                        <td valign="top" nowrap="nowrap"><input type="checkbox" name="delivery_id[]"
                                                                value="{{ $delivery['delivery_id'] }}"/>{{ $delivery['delivery_sn'] }}
                        </td>
                        <td>{{ $delivery['order_sn'] }}<br/></td>
                        <td align="center" valign="top" nowrap="nowrap">{{ $delivery['add_time'] }}</td>
                        <td align="left" valign="top"><a href="mailto:{{ $delivery['email'] }}">
                                {{ $delivery['consignee'] }}</a></td>
                        ...
                        <td align="center" valign="top" nowrap="nowrap">
                            <a
                                href="order.php?act=delivery_info&delivery_id={{ $delivery['delivery_id'] }}">{{ $lang['detail'] }}</a>
                            <a onclick="{if(confirm('{{ $lang['confirm_delete'] }}')){return true;}return false;}"
                               href="order.php?act=operate&remove_invoice=1&delivery_id={{ $delivery['delivery_id'] }}">{{ $lang['remove'] }}</a>
                        </td>
                    </tr>
                @endforeach
            </table>
...
@if($full_page)
        ...
        <div>
            ...
            <input name="remove_invoice" type="submit" id="btnSubmit3" value="{{ $lang['remove'] }}" class="button"
