...
<div class="list-div">
    <form method="post" action="captcha_manage.php" name="theForm" onsubmit="return validate()">
        <table cellpadding='3' cellspacing='1'>
            ...
            <tr>
                ...
                <td>
                    <input type="checkbox" name="captcha_register"
                           value="1" {{ $captcha['register'] }} />{{ $lang['captcha_register'] }}<br/>
                    <input type="checkbox" name="captcha_login"
                           value="2" {{ $captcha['login'] }} />{{ $lang['captcha_login'] }}<br/>
                    <input type="checkbox" name="captcha_comment"
                           value="4" {{ $captcha['comment'] }} />{{ $lang['captcha_comment'] }}<br/>
                    <input type="checkbox" name="captcha_admin"
                           value="8" {{ $captcha['admin'] }} />{{ $lang['captcha_admin'] }}<br/>
                    <input type="checkbox" name="captcha_message"
                           value="32" {{ $captcha['message'] }} />{{ $lang['captcha_message'] }}<br/>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td><input type="radio" name="captcha_login_fail"
                           value="32" {{ $captcha['login_fail_yes'] }} />{{ $lang['yes'] }}<input type="radio"
                                                                                                  name="captcha_login_fail"
                                                                                                  value="0" {{ $captcha['login_fail_no'] }} />{{ $lang['no'] }}
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                <td colspan="2" align="center"><input type="hidden" name="act" value="save_config"/><input type="submit"
                                                                                                           value="保存设置"
                                                                                                           class="button"/>
                </td>
            </tr>
