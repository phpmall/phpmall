...
<div class="list-div" style="margin-bottom: 5px">
    <table id="general-table" align="center" width="100%" cellpadding="3" cellspacing="1" border="0">
        <tr>
            <td colspan="2" width="100%"><a href="javascript:showNotice('noticeGoodsSN');"
                                            title="{{ $lang['form_notice'] }}"><img
                        src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                        alt="{{ $lang['form_notice'] }}"></a><strong>请选择模板的模式：</strong><input type="radio"
                                                                                                      name="model"
                                                                                                      id="model_1"
                                                                                                      value="1"
                '.($shipping['print_model'] === 1 ? 'checked="checked"@endif
                onclick="javascript:model_change('1');">代码模式&nbsp;&nbsp;&nbsp;&nbsp;<input
                    type="radio" name="model" id="model_2" value="2"
                    @if($shipping['print_model'] === 2)checked="checked"
                    @endif onclick="javascript:model_change('2');">所见即所得模式<br/><span class="notice-span"
                                                                                            @if($help_open)style="display:block"
                ' : ' style="display:none" ').'
                id="noticeGoodsSN">选择“代码模式”可以切换到以前版本。建议您使用“所见即所得模式”。所有模式选择后，同样在打印模板中生效。</span>
            </td>
        </tr>
        <tr>
        ...
        <tr id="visual" @if($shipping['print_model'] === 1)style="display:none"@endif>
            <td colspan="2" width="100%"><!--Flash播放器 start-->
                <iframe id="goods_desc___Frame" src="shipping.php?act=print_index&shipping={{ $shipping_id }}"
                        width="99%" height="675" frameborder="0" scrolling="no"></iframe><!--Flash播放器 end--></td>
        </tr>

        <form method="post" name="theForm"
              action="shipping.php?act=do_edit_print_template&shipping={{ $shipping['shipping_id'] }}">
            ...
            <tr id="code_shipping_print" @if($shipping['print_model'] === 2)style="display:none"@endif>
                <td width="75%"><textarea id="shipping_print" name="shipping_print" rows="26"
                                          cols="100">{{ $shipping['shipping_print'] }}</textarea></td>
            ...
            <tr id="code_submit" @if($shipping['print_model'] === 2)style="display:none"@endif>
                <td colspan="2" align="center" width="100%"><input type="submit" value="{{ $lang['button_submit'] }}"
                                                                   class="button"/></td>
            </tr>
        </form>
