    ...
    <div class="form-div">
        ...
        @else
            <form action="affiliate_ck.php?act=list">
                ...
                <input name="order_sn" type="text" id="order_sn" size="15"><input type="submit"
        ...
        <div class="list-div" id="listDiv">
            ...
            <table cellspacing='1' cellpadding='3'>
                ...
                @forelse($logdb as $val)
                    <tr>
                        <td align="center"><a
                                href="order.php?act=info&order_id={{ $val['order_id'] }}">{{ $val['order_sn'] }}</a>
                        </td>
                        ...
                        <td>
                            @if($val['is_separate'] === 0 && $val['separate_able'] === 1 && $on === 1)
                                <a
                                    href="javascript:confirm_redirect(separate_confirm, 'affiliate_ck.php?act=separate&oid={{ $val['order_id'] }}')">{{ $lang['affiliate_separate'] }}</a>
                                | <a
                                    href="javascript:confirm_redirect(cancel_confirm, 'affiliate_ck.php?act=del&oid={{ $val['order_id'] }}')">{{ $lang['affiliate_cancel'] }}</a>
                            @elseif($val['is_separate'] === 1)
                                <a
                                    href="javascript:confirm_redirect(rollback_confirm, 'affiliate_ck.php?act=rollback&logid={{ $val['log_id'] }}')">{{ $lang['affiliate_rollback'] }}</a>
                            @else
                                -
                            @endif
                        </td>
