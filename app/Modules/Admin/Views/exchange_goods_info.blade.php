    ...
    <form action="exchange_goods.php" method="post" name="theForm" onsubmit="return validate();">
        <table width="90%" id="general-table">
            <tr>
                ...
                <td><input type="text" name="keywords" size="30"/>
                    <input type="button" value="{{ $lang['button_search'] }}" class="button" onclick="searchGoods()"
                           @if($form_action === 'update') disabled="true" @endif></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input type="text" name="exchange_integral" maxlength="60" size="20"
                           value="{{ $goods['exchange_integral'] }}"/>{{ $lang['require_field'] }}<br/><span
                        class="notice-span"
                        {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticepackagePrice">{{ $lang['notice_integral'] }}</span>
                </td>
            </tr>
            <tr>
                <td class="narrow-label">{{ $lang['is_exchange'] }}</td>
                <td>
                    <input type="radio" name="is_exchange" value="1"
                           @if($goods['is_exchange'] === 1)checked@endif> {{ $lang['isexchange'] }}
                    <input type="radio" name="is_exchange" value="0"
                           @if($goods['is_exchange'] === 0)checked@endif> {{ $lang['isnotexchange'] }}{{ $lang['require_field'] }}
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <input type="radio" name="is_hot" value="1"
                           @if($goods['is_hot'] === 1)checked@endif> {{ $lang['ishot'] }}
                    <input type="radio" name="is_hot" value="0"
                           @if($goods['is_hot'] === 0)checked@endif> {{ $lang['isnothot'] }}{{ $lang['require_field'] }}
                </td>
            </tr>
        </table>
