        ...
        <table>
            ...
            @if($show_refund)
                <tr>
                    ...
                    <td>
                        <p>@if(!$anonymous)
                                <label><input type="radio" name="refund" value="1"/>{{ $lang['return_user_money'] }}
                                </label><br>
                            @endif
                            <label><input type="radio" name="refund" value="2"/>{{ $lang['create_user_account'] }}
                            </label><br>
                ...
                <tr>
                    ...
                    <td><textarea name="refund_note" cols="60" rows="3" id="refund_note">{{ $refund_note }}</textarea>
                    </td>
                </tr>
            @endif
            <tr>
                <td colspan="2">
                    <div align="center">
                        ...
                        <input type="button" name="back" value="{{ $lang['back'] }}" class="button"
                               onclick="history.back()"/>
                        <input type="hidden" name="order_id" value="{{ $order_id }}"/>
                        <input type="hidden" name="operation" value="{{ $operation }}"/>
                        <input type="hidden" name="act" value="'.($batch ? 'batch_operate_post' : 'operate_post').'"/>
                    </div>
                </td>
            </tr>
        </table>
