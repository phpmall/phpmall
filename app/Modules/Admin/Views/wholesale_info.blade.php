...
<div class="main-div">
    <form method="post" action="wholesale.php" name="theForm" enctype="multipart/form-data"
        ...
        <table width="100%">
            <!-- @if($form_action === "insert")
                ...
                <tr>
                    ...
                    <td><!-- 分类 -->
                        <select name="cat_id">
                            <option value="0">{{ $lang['custom_goods_cat'] }}</option>{{ $cat_list }}</select>
                        <!-- 品牌 -->
                        <select name="brand_id">
                            <option value="0">{{ $lang['custom_goods_brand'] }}</option>
                            {html_options options=$brand_list}</select>
                        <!-- 关键字 -->
                        {{ $lang['label_search_goods'] }}<input name="keyword" type="text" id="keyword" size="10">
                        <!-- 搜索 -->
                        <input name="search" type="button" id="search" value="{{ $lang['button_search'] }}"
                               class="button" onclick="searchGoods()"/></td>
                </tr>
            @endif
            <tr>
                <td class="label">{{ $lang['label_goods_name'] }}</td>
                <td><select name="goods_id" id="goods_id"
                            onchange="document.getElementById('price-div').innerHTML = ''; getGoodsInfo(this.value);">
                        <option value="{{ $wholesale['goods_id'] }}"
                                selected="selected">{{ $wholesale['goods_name'] }}</option>
                    </select>
            ...
            <tr>
                ...
                <td>@foreach($user_rank_list as $rank)
                        <input name="rank_id[]" type="checkbox" id="rank_id[]" value="{{ $rank['rank_id'] }}"
                               @if($rank['checked'])checked="checked"@endif />
                        {{ $rank['rank_name'] }}
                    @endforeach</td>
            </tr>
            <tr>
                ...
                <td><label>
                        <input type="radio" name="enabled" value="1"
                               @if($wholesale['enabled'])checked="checked"@endif />
                        {{ $lang['yes'] }}</label>
                    <label>
            ...
            @foreach($wholesale['price_list'] as $key => $attr_price)
                <table width="100%">
                    <!-- @if($attr_list)
                        ...
                        <tr>
                            ...
                            <td colspan="2">
                                @foreach($attr_list as $attr)
                                    {{ $attr['attr_name'] }} <select name="attr_{{ $attr['attr_id'] }}[{{ $key }}]">
                                        {html_options options=$attr.goods_attr_list
                                        selected=$attr_price.attr[$attr.attr_id]} </select>
                                @endforeach
                            </td>
                            <td>'.($key === 0 ? ' <input type="button" class="button" value=" + "
                                                         onclick="addAttr(this)"/> ' : ' <input type="button"
                                                                                                class="button"
                                                                                                value=" - "
                                                                                                onclick="dropAttr(this)"/>
                                ').'
                            </td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td></td>
                            <td colspan="3" style="border-bottom:1px #000 dashed;"></td>
                            <td></td>
                        </tr>
                    @endif
                    ...
                    @foreach($attr_price['qp_list'] as $index => $qp)
                        <tr>
                            <td width="10%">&nbsp;</td>
                            <td> {{ $lang['quantity'] }} <input name="quantity[{{ $key }}][]" type="text"
                                                                value="{{ $qp['quantity'] }}"/></td>
                            <td> {{ $lang['price'] }} <input name="price[{{ $key }}][]" type="text"
                                                             value="{{ $qp['price'] }}"/></td>
                            <td> '.($index === 0 ? ' <input type="button" class="button" value=" + "
                                                            onclick="addQuantityPrice(this, '{{ $key }}')"/> ' : '
                                <input
                                    type="button" class="button" value=" - " onclick="dropQuantityPrice(this)"/> ').'
                            </td>
                            <td width="10%">&nbsp;</td>
                        </tr>
                    @endforeach
