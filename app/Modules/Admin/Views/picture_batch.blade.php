...
<div class="main-div">
    <form action="picture_batch.php" method="post" name="theForm" onsubmit="return start()">
        <table cellspacing="1" cellpadding="3" width="100%">
            ...
            <tr>
                <td><select name="cat_id" onchange="goods_list(this);">
                    ...
                    <span id="list"><select name="goods_id">
                            <option value="0">{{ $lang['all_goods'] }}</option>
                        </select></span>
            ...
            <tr>
                <td>
                    <label for="do_icon"><input type="checkbox" name="do_icon" value="1" id="do_icon"
                                                checked="true"/>{{ $lang['do_icon'] }}</label>
                    <label for="do_album"><input type="checkbox" name="do_album" value="1" id="do_album"
                                                 checked="true"/>{{ $lang['do_album'] }}</label>
                </td>
            </tr>
            <tr>
                <td><label for="process_thumb"><input type="checkbox" name="process_thumb" value="1" id="process_thumb"
                                                      checked="true"/>{{ $lang['thumb'] }}</label></td>
            </tr>
            <tr>
                <td><label for="process_watermark"><input type="checkbox" name="process_watermark" value="1"
                                                          id="process_watermark"
                                                          checked="true"/>{{ $lang['watermark'] }}</label></td>
            </tr>
            <tr>
                <td>
                    <label for="yes_change"><input type="radio" name="change_link" value="1"
                                                   id="yes_change"/>{{ $lang['yes_change'] }}</label>
                    <label for="no_change"><input type="radio" name="change_link" value="0" checked="true"
                                                  id="no_change"/>{{ $lang['no_change'] }}</label>
                </td>
            </tr>
            <tr>
                <td>
                    <label for="silent"><input type="radio" name="silent" value="1" id="silent"
                                               checked="checked"/>{{ $lang['silent'] }}</label>
                    <label for="no_silent"><input type="radio" name="silent" value="0"
                                                  id="no_silent"/>{{ $lang['no_silent'] }}</label>
                </td>
            </tr>
            <tr>
                <td align="center">
...
<div
    style="display:none;border: 1px solid rgb(204, 0, 0);margin-top:10px; padding: 4px; background-color: rgb(255, 255, 206); color: rgb(206, 0, 0);"
    id="errorMsg"></div>
...
<script type="text/javascript">
    ...
    function add_search_goods(obj) {
        ...
        goods_div.innerHTML = '<input type="hidden" name="multi_goods_id[]" value="' + goods_id + '">' + goods_name + '&nbsp;&nbsp;<img style="cursor: pointer;" onclick="del_search_goods(\'' + 'goods_' + goods_id + '\');" src="{{ asset('static/admin/images/no.gif') }}"/>';
