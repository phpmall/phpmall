...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="table_box">
    <form action="shipping.php" enctype="multipart/form-data" method="post" name="theForm" id="theForm"
        ...
        <tr>
            <td style="overflow:hidden; background-color:#EFEFDE; padding-left:8px;">
                <table width="100%" cellpadding="0" cellspacing="0" border="0" height="50">
                    <tr>
                        <td align="left">
                            <select name="lable" id="lable" class="select_box"
                                    onchange="javascript:call_flash('lable_add', this);">
                                <option value="" selected="selected">{{ $lang['lable_select_notice'] }}</option>
                                {html_options options=$lang.lable_box}
                            </select>
                            <input type="button" name="del" id="del" value="{{ $lang['del_lable'] }}"
                                   onclick="javascript:call_flash('lable_del', this);">
                        </td>
                        <td id="pic_control_upload" @if($shipping['print_bg'] != '') class="display_no" @endif>
                            <input type="file" name="bg" id="bg"
                                   @if($shipping['print_bg'] != '') disabled="disabled" @endif>
                            <input type="button" name="upload" id="upload" value="{{ $lang['upload_shipping_bg'] }}"
                                   onclick="javascript:bg_upload();"
                                   @if($shipping['print_bg'] != '') disabled="disabled"
                                @endif>
                            <iframe id="bg_upload_hidden" name="bg_upload_hidden" frameborder="0" scrolling="no"
                                    class="display_no"></iframe>
                        </td>
                        <td id="pic_control_del" @if($shipping['print_bg'] === '') class="display_no" @endif>
                            <input type="button" name="upload_del" id="upload_del" onclick="javascript:bg_del();"
                                   value="{{ $lang['del_shipping_bg'] }}"
                                   @if($shipping['print_bg'] === '') disabled="disabled"
                                @endif>
                        </td>
                        <td align="right"><input type="button" value="{{ $lang['save_setting'] }}"
                                                 onclick="javascript:save();">&nbsp;&nbsp;<input type="button"
                                                                                                 value="{{ $lang['recovery_default'] }}"
                                                                                                 onclick="javascript:recovery_default();">
                        </td>
                    </tr>
                    <tr>
                        <td colspan="3">&nbsp;</td>
                ...
                <div class="div_play_aray">
                    <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"
                        ...
                        <param name="FlashVars"
                               value="bcastr_config_bg={{ $shipping['print_bg'] }}&swf_config_lable={{ $shipping['config_lable'] }}">
                        <param name="allowScriptAccess" value="sameDomain"/>
                        <embed src="../data/print/pint.swf" wmode="transparent"
