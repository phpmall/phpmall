    ...
    <form method="post" action="users.php" name="theForm" onsubmit="return validate()">
        <table width="100%">
            <tr>
                ...
                <td>'.($form_action === "update" ? '{{ $user['user_name'] }}<input type="hidden" name="username"
                                                                                   value="{{ $user['user_name'] }}"/>' :
                    '<input type="text" name="username" maxlength="60"
                            value="{{ $user['user_name'] }}"/>{{ $lang['require_field'] }}').'
                </td>
            </tr>
            @if($form_action === "update")
                <tr>
                    <td class="label">{{ $lang['user_money'] }}:</td>
                    <td>{{ $user['formated_user_money'] }} <a
                            href="account_log.php?act=list&user_id={{ $user['user_id'] }}&account_type=user_money">[
                            {{ $lang['view_detail_account'] }}
                            ]</a></td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['frozen_money'] }}:</td>
                    <td>{{ $user['formated_frozen_money'] }} <a
                            href="account_log.php?act=list&user_id={{ $user['user_id'] }}&account_type=frozen_money">[
                            {{ $lang['view_detail_account'] }}
                            ]</a></td>
                </tr>
                <tr>
                    <td class="label"><a href="javascript:showNotice('noticeRankPoints');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}"
                                width="16" height="16" border="0" alt="{{ $lang['form_notice'] }}"></a>
                        {{ $lang['rank_points'] }}:
                    </td>
                    <td>{{ $user['rank_points'] }} <a
                            href="account_log.php?act=list&user_id={{ $user['user_id'] }}&account_type=rank_points">[
                            {{ $lang['view_detail_account'] }}
                            ]</a> <br/><span class="notice-span"
                                             {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeRankPoints">{{ $lang['notice_rank_points'] }}</span>
                    </td>
                </tr>
                <tr>
                    <td class="label"><a href="javascript:showNotice('noticePayPoints');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}"
                                width="16" height="16" border="0" alt="{{ $lang['form_notice'] }}"/></a>
                        {{ $lang['pay_points'] }}:
                    </td>
                    <td>{{ $user['pay_points'] }} <a
                            href="account_log.php?act=list&user_id={{ $user['user_id'] }}&account_type=pay_points">[
                            {{ $lang['view_detail_account'] }}
                            ]</a> <br/>
                        <span class="notice-span" {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }}
                        id="noticePayPoints">{{ $lang['notice_pay_points'] }}</span>
                    </td>
                </tr>
            @endif
            <tr>
                <td class="label">{{ $lang['email'] }}:</td>
                <td><input type="text" name="email" maxlength="60" size="40"
                           value="{{ $user['email'] }}"/>{{ $lang['require_field'] }}</td>
            </tr>
            @if($form_action === "insert")
                <tr>
                    <td class="label">{{ $lang['password'] }}:</td>
                    <td><input type="password" name="password" maxlength="20" size="20"/>{{ $lang['require_field'] }}
                    </td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['confirm_password'] }}:</td>
                    <td><input type="password" name="confirm_password" maxlength="20"
                               size="20"/>{{ $lang['require_field'] }}</td>
                </tr>
            ...
            <tr>
                ...
                <td>{html_select_date field_order="YMD" prefix="birthday" time=$user.birthday start_year="-60"
                    end_year="+1" display_days=true month_format="%m"}
                </td>
            </tr>
            <tr>
                <td class="label">{{ $lang['credit_line'] }}:</td>
                <td><input name="credit_line" type="text" id="credit_line" value="{{ $user['credit_line'] }}"
                           size="10"/></td>
            </tr>
            @foreach($extend_info_list as $field)
                <tr>
                    ...
                    <td>
                        <input name="extend_field{{ $field['id'] }}" type="text" size="40" class="inputBg"
                               value="{{ $field['content'] }}"/>
                    </td>
                </tr>
            @endforeach
            @if($user['parent_id'])
                <tr>
                    ...
                    <td><a
                            href="users.php?act=edit&id={{ $user['parent_id'] }}">{{ $user['parent_username'] }}</a>&nbsp;&nbsp;&nbsp;&nbsp;<a
                            href="users.php?act=remove_parent&id={{ $user['user_id'] }}">{{ $lang['parent_remove'] }}</a>
                    </td>
                </tr>
            @endif
            @if($affiliate['on'] === 1 && $affdb)
                <tr>
                    ...
                    <td>
                        [<a
                            href="users.php?act=aff_list&auid={{ $user['user_id'] }}">{{ $lang['show_affiliate_users'] }}</a>][<a
                            href="affiliate_ck.php?act=list&auid={{ $user['user_id'] }}">{{ $lang['show_affiliate_orders'] }}</a>]
                    </td>
                </tr>
                <tr>
                    <td></td>
