...
<div class="main-div">
    <form action="attribute.php" method="post" name="theForm" onsubmit="return validate();">
        <table width="100%" id="general-table">
            ...
            <tr>
                <td class="label"><a href="javascript:showNotice('noticeindex');"
                                     title="{{ $lang['form_notice'] }}"><img
                            src="{{ asset('static/admin/images/notice.gif') }}"
                            width="16" height="16" border="0"
                            alt="{{ $lang['form_notice'] }}"></a>{{ $lang['label_attr_index'] }}</td>
                <td>
                    <input type="radio" name="attr_index" value="0" @if($attr['attr_index'] === 0) checked="true"
                        @endif />
                    {{ $lang['no_index'] }}
                    <input type="radio" name="attr_index" value="1" @if($attr['attr_index'] === 1) checked="true"
                        @endif />
                    {{ $lang['keywords_index'] }}
                    <input type="radio" name="attr_index" value="2" @if($attr['attr_index'] === 2) checked="true"
                        @endif />
                    {{ $lang['range_index'] }}
                    <br/><span class="notice-span"
                               {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeindex">{{ $lang['note_attr_index'] }}</span>
                </td>
            </tr>
            <tr>
                <td class="label">{{ $lang['label_is_linked'] }}</td>
                <td>
                    <input type="radio" name="is_linked" value="0"
                           @if($attr['is_linked'] === 0) checked="true" @endif />
                    {{ $lang['no'] }}
                    <input type="radio" name="is_linked" value="1"
                           @if($attr['is_linked'] === 1) checked="true" @endif />
                    {{ $lang['yes'] }}
                </td>
            </tr>
            <tr>
                <td class="label"><a href="javascript:showNotice('noticeAttrType');"
                                     title="{{ $lang['form_notice'] }}"><img
                            src="{{ asset('static/admin/images/notice.gif') }}"
                            width="16" height="16" border="0"
                            alt="{{ $lang['form_notice'] }}"></a>{{ $lang['label_attr_type'] }}</td>
                <td>
                    <input type="radio" name="attr_type" value="0"
                           @if($attr['attr_type'] === 0) checked="true" @endif />
                    {$lang.attr_type_values[0]}
                    <input type="radio" name="attr_type" value="1"
                           @if($attr['attr_type'] === 1) checked="true" @endif />
                    {$lang.attr_type_values[1]}
                    <input type="radio" name="attr_type" value="2"
                           @if($attr['attr_type'] === 2) checked="true" @endif />
                    {$lang.attr_type_values[2]}
                    <br/><span class="notice-span"
                               {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeAttrType">{{ $lang['note_attr_type'] }}</span>
                </td>
            </tr>
            <tr>
                <td class="label">{{ $lang['label_attr_input_type'] }}</td>
                <td>
                    <input type="radio" name="attr_input_type" value="0" @if($attr['attr_input_type'] === 0)
                        checked="true" @endif onclick="radioClicked(0)"/>
                    {{ $lang['text'] }}
                    <input type="radio" name="attr_input_type" value="1" @if($attr['attr_input_type'] === 1)
                        checked="true" @endif onclick="radioClicked(1)"/>
                    {{ $lang['select'] }}
                    <input type="radio" name="attr_input_type" value="2" @if($attr['attr_input_type'] === 2)
                        checked="true" @endif onclick="radioClicked(0)"/>
                    {{ $lang['text_area'] }}
                </td>
            </tr>
