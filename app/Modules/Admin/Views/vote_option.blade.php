        ...
        <table cellspacing='1' cellpadding='3' id='listTable'>
            ...
            @foreach($option_arr as $list)
                <tr align="center">
                    ...
                    <span
                        onclick="javascript:listTable.edit(this, 'edit_option_name', {{ $list['option_id'] }})">{$list.option_name|escape:"html"}</span>
                    </td>
                    <td><span
                            onclick="javascript:listTable.edit(this, 'edit_option_order', {{ $list['option_id'] }})">{{ $list['option_order'] }}</span>
                    </td>
                    <td>{{ $list['option_count'] }}</td>
                    <td><a href="javascript:;"
                           onclick="listTable.remove({{ $list['option_id'] }}, '{{ $lang['drop_confirm'] }}', 'remove_option')"
