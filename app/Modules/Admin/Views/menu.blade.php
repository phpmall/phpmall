...
<head>
    ...
    <script type="text/javascript">
        var noHelp = "<p align='center' style='color: #666'>{{ $no_help }}</p > ";
    ...
    <style type="text/css">
        ...
        .explode {
            background: url({{ asset('static/admin/images/menu_minus.gif') }}) no-repeat 0px 3px;
            font-weight: bold;
        }

        .collapse {
            background: url({{ asset('static/admin/images/menu_plus.gif') }}) no-repeat 0px 3px;
            font-weight: bold;
        }

        .menu-item {
            background: url({{ asset('static/admin/images/menu_arrow.gif') }}) no-repeat 0px 3px;
            font-weight: normal;
        }
...
<div id="tabbar-div">
    ...
    <span style="float:right; padding: 3px 5px;">
        <a href="javascript:toggleCollapse();">
            <img id="toggleImg" src="{{ asset('static/admin/images/menu_minus.gif') }}" width="9" height="9" border="0"
                 alt="{{ $lang['collapse_all'] }}"/>
        </a>
    </span>
        ...
        <ul id="menu-ul">
            @foreach ($menus as $k => $menu)
                @if (isset($menu['action']))
                    <li class="explode"><a href="{{ $menu['action'] }}" target="main-frame">{{ $menu['label'] }}</a>
                    </li>
                @else
                        ...
                        @if ($menu['children'])
                            <ul>
                                @foreach ($menu['children'] as $child)
                                    <li class="menu-item"><a href="{{ $child['action'] }}"
                                                             target="main-frame">{{ $child['label'] }}</a></li>
                                @endforeach
                            </ul>
                        @endif
                    </li>
...
<script type="text/javascript" src="{{ asset('static/admin/js/menu.js') }}"></script>
<script type="text/javascript">
    ...
    function toggleCollapse() {
        ...
        collapse = !collapse;
        document.getElementById('toggleImg').src = collapse ? '{{ asset("static/admin/images/menu_minus.gif") }}' : '{{ asset("static/admin/images/menu_plus.gif") }}';
