        ...
        <form action="article.php" method="post" enctype="multipart/form-data" name="theForm"
            ...
            <table width="90%" id="general-table">
                <tr>
                    ...
                    <td><input type="text" name="title" size="40" maxlength="60"
                ...
                @if($article['cat_id'] >= 0)
                    <tr>
                        ...
                        <td><input type="radio" name="article_type" value="0"
                                   @if($article['article_type'] === 0)checked@endif>{{ $lang['common'] }}
                            <input type="radio" name="article_type" value="1"
                                   @if($article['article_type'] === 1)checked@endif>{{ $lang['top'] }}
                            {{ $lang['require_field'] }}        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input type="radio" name="is_open" value="1"
                                   @if($article['is_open'] === 1)checked@endif> {{ $lang['isopen'] }}
                            <input type="radio" name="is_open" value="0"
                                   @if($article['is_open'] === 0)checked@endif> {{ $lang['isclose'] }}{{ $lang['require_field'] }}
                        </td>
                    </tr>
                @else
                    <tr style="display:none">
                        <td colspan="2"><input type="hidden" name="article_type" value="0"/><input type="hidden"
                                                                                                   name="is_open"
                                                                                                   value="1"/></td>
                    </tr>
                @endif
                ...
                <tr>
                    ...
                    <td><input type="text" name="author_email" maxlength="60" value="{{ $article['author_email'] }}"/>
                    </td>
                </tr>
                <tr>
            ...
            <table width="90%" id="goods-table" style="display:none">
                ...
                <tr>
                    <td colspan="5">
                        ...
                        <!-- 分类 -->
                        <select name="cat_id">
                            <option value="0">{{ $lang['all_category'] }}</caption>{{ $goods_cat_list }}</select>
                        <!-- 品牌 -->
                        ...
                        <input type="button" value="{{ $lang['button_search'] }}" onclick="searchGoods()"
                               class="button"/>
                    <td>
                </tr>
                <!-- 商品列表 -->
                ...
                <tr>
                    <td width="45%" align="center">
                        <select name="source_select" size="20" style="width:90%"
                                ondblclick="sz.addItem(false, 'add_link_goods', articleId)" multiple="true">
                        </select>
                    </td>
                    <td align="center">
                        <p><input type="button" value="&gt;&gt;" onclick="sz.addItem(true, 'add_link_goods', articleId)"
                                  class="button"/></p>
                        <p><input type="button" value="&gt;" onclick="sz.addItem(false, 'add_link_goods', articleId)"
                                  class="button"/></p>
                        <p><input type="button" value="&lt;" onclick="sz.dropItem(false, 'drop_link_goods', articleId)"
                                  class="button"/></p>
                        <p><input type="button" value="&lt;&lt;"
                                  onclick="sz.dropItem(true, 'drop_link_goods', articleId)" class="button"/></p>
                    </td>
                    <td width="45%" align="center">
                        <select name="target_select" multiple="true" size="20" style="width:90%"
                                ondblclick="sz.dropItem(false, 'drop_link_goods', articleId)">
