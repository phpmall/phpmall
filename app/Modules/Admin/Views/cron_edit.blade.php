    ...
    <div class="main-div">
        <table cellspacing="1" cellpadding="3" width="100%">
            ...
            <tr>
                ...
                <td>
                    <input name="cron_name" type="text" value="{{ $cron['cron_name'] }}" size="40" readonly="readonly"
                           style="background:#dddddd"/>
                </td>
            </tr>
            <tr>
                <td class="label">{{ $lang['cron_desc'] }}</td>
                <td><textarea name="cron_desc" cols="80" rows="5" style="" readonly="readonly"
                              style="background:#dddddd">{{ $cron['cron_desc'] }}</textarea></td>
            </tr>
            @foreach($cron['cron_config'] as $config)
                <tr>
                    <td class="label">{{ $config['label'] }}</td>
                    @if($config['type'] === "text")
                        <td><input name="cfg_value[]" type="{{ $config['type'] }}" value="{{ $config['value'] }}"
                                   size="40"/>
                        </td>
                    @elseif($config['type'] === "textarea")
                        <td><textarea name="cfg_value[]" cols="80" rows="5">{{ $config['value'] }}</textarea></td>
                    @elseif($config['type'] === "select")
                        <td><select name="cfg_value[]">{html_options options=$config.range
                                selected=$config.value}</select></td>
                    @endif
            ...
            <tr>
                ...
                <td>
                    <input type="radio" name="ttype" id="ttype_day" value="day"
                           onClick="dotype('day');"/>{{ $lang['cron_month'] }}
                    <select name="cron_day" id="cron_day" disabled="true">{html_options options=$days
                        selected=$cron.cronday}</select>&nbsp;{{ $lang['cron_day'] }}&nbsp;
                    <input type="radio" name="ttype" id="ttype_week" value="week"
                           onClick="dotype('week');"/>{{ $lang['cron_week'] }}
                    <select name="cron_week" id="cron_week" disabled="true">{html_options options=$week
                        selected=$cron.cronweek}</select>
                    <input type="radio" name="ttype" id="ttype_unlimit" value="unlimit"
                           onClick="dotype('unlimit')">{{ $lang['cron_unlimit'] }}
                    <br/>
                    {{ $lang['cron_thatday'] }}<select name="cron_hour">{html_options options=$hours
                        selected=$cron.cronhour}</select>&nbsp;{{ $lang['cron_hour'] }}&nbsp;
                </td>
            </tr>
            <tr>
                <td class="label"><a href="javascript:showNotice('notice_minute');"
                                     title="{{ $lang['form_notice'] }}"><img
                            src="{{ asset('static/admin/images/notice.gif') }}"
                            width="16" height="16" border="0"
                            alt="{{ $lang['form_notice'] }}"></a>{{ $lang['cron_minute'] }}</td>
            ...
            <tr>
                ...
                <td>
                    ...
                    id="notice_minute">{{ $lang['notice_minute'] }}</span>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><label><input name="show_advance" id="show_advance" type="checkbox" value="1"
                                  onClick="show();"/>{{ $lang['cron_show_advance'] }}</label></td>
            </tr>
            <tr id="advance_1">
                <td class="label"><a href="javascript:showNotice('notice_alow_ip');"
                                     title="{{ $lang['form_notice'] }}"><img
                            src="{{ asset('static/admin/images/notice.gif') }}"
                            width="16" height="16" border="0"
                            alt="{{ $lang['form_notice'] }}"></a>{{ $lang['cron_allow_ip'] }}</td>
            ...
            <tr id="advance_2">
                ...
                <td>
                    <span class="notice-span" {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }}
                    id="notice_alow_ip">{{ $lang['notice_alow_ip'] }}</span>
                </td>
            </tr>
            <tr id="advance_3">
                <td class="label"><a href="javascript:showNotice('notice_alow_files');"
                                     title="{{ $lang['form_notice'] }}"><img
                            src="{{ asset('static/admin/images/notice.gif') }}"
                            width="16" height="16" border="0"
                            alt="{{ $lang['form_notice'] }}"></a>{{ $lang['cron_alow_files'] }}</td>
                <td>
                    @foreach($page_list as $page_list => $list)
                        <div style="width:200px;float:left;">
                            <label><input type="checkbox" name="alow_files[]" value="{{ $page_list }}.php"
                                          id="{{ $page_list }}" class="checkbox"
                                          @if($list === 1) checked="true" @endif />
            ...
            <tr id="advance_4">
                ...
                <td>
                    <span class="notice-span" {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }}
                    id="notice_alow_files">{{ $lang['notice_alow_files'] }}</span>
                </td>
            </tr>
            <tr align="center">
                <td colspan="2">
