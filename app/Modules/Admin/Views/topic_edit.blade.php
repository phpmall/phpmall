...
<script src="{{ asset('static/admin/js/colorselector_topic.js') }}"></script>
...
<div class="tab-div">
    <!-- tab bar -->
    <div id="tabbar-div">
        <p><span class="tab-front" id="general-tab">{{ $lang['tab_general'] }}</span> <span class="tab-back"
                                                                                            id="goods-tab">{{ $lang['tab_goods'] }}</span>
            <span class="tab-back" id="desc-tab">{{ $lang['tab_desc'] }}</span><span class="tab-back"
                                                                                     id="advanced-tab">{{ $lang['tab_advanced'] }}</span>
        </p>
    </div>
    <!-- tab body -->
    <div id="tabbody-div">
        <form action="topic.php" method="post" name="theForm" enctype="multipart/form-data">
            <table cellspacing="1" id="general-table" cellpadding="3" width="100%">
                ...
                <tr>
                    ...
                    <td><textarea name="keywords" id="keywords" cols="40" rows="3">{{ $topic['keywords'] }}</textarea>
                    </td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['lable_topic_description'] }}</td>
                    <td><textarea name="description" id="description" cols="40"
                                  rows="5">{{ $topic['description'] }}</textarea></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class="label">
                        <a href="javascript:showNotice('title_upload');" title="{{ $lang['form_notice'] }}">
                            <img src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16"
                                 border="0" alt="{{ $lang['form_notice'] }}"></a>{{ $lang['lable_upload'] }}
                    </td>
                    <td>
                        <input type='file' name='topic_img' id='topic_img' size='35'/>
                        <br/><span class="notice-span"
                                   {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="title_upload">{{ $width_height }}</span>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class="label">&nbsp;</td>
                    <td><input type="text" name="img_url" id="img_url" value="{{ $topic['topic_img'] }}" size="35"
                ...
                <tr>
                    <td class="label"><a href="javascript:showNotice('title_pic_upload');"
                                         title="{{ $lang['form_notice'] }}">
                            <img src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                 alt="{{ $lang['form_notice'] }}"></a>{{ $lang['lable_title_upload'] }}</td>
                    <td><input type='file' name='title_pic' id='title_pic' size='35'/>
                        <br/><span class="notice-span"
                                   {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="title_pic_upload">{{ $title_width_height }}</span>
                    </td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['lable_from_web'] }}</td>
                ...
                <tr>
                    <td class="label">&nbsp;</td>
                    <td><input type="text" name="title_img_url" id="title_img_url" value="{{ $topic['title_pic'] }}"
                ...
                <tr>
                    ...
                    <td><input type="text" name="base_style" id="base_style" value="{{ $topic['base_style'] }}" size="7"
                               maxlength="6" style="float:left;color:{{ $goods_name_color }};" size="30"/>
                        <div style="background-color:#{{ $topic['base_style'] }};float:left;margin-left:2px;"
                ...
                <tr>
                    ...
                    <td><input name="start_time" type="text" id="start_time" size="12"
                               value='{{ $topic['start_time'] }}' readonly="readonly"/>
                        ...
                        -
                        <input name="end_time" type="text" id="end_time" size="12" value='{{ $topic['end_time'] }}'
                               readonly="readonly"/>
                        <input name="selbtn2" type="button" id="selbtn2"
                               onclick="return showCalendar('end_time', '%Y-%m-%d', false, false, 'selbtn2');"
                               value="{{ $lang['btn_select'] }}" class="button"/>
                    </td>
                </tr>
            </table>
            <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" id="goods-table"
                ...
                <tr>
                    <td colspan="4" class="label" style="text-align:left">{{ $lang['topic_class'] }}
                        ...
                        <input name="create_class_btn" type="button" id="create_class_btn" value="{{ $lang['add'] }}"
                               class="button" onclick="addClass()"/>
                        <input name="delete_class_btn" type="button" id="delete_class_btn" value="{{ $lang['remove'] }}"
                               class="button" onclick="deleteClass()"/>
                    </td>
                </tr>
                <tr>
                    <td colspan="3"><img src="{{ asset('static/admin/images/icon_search.gif') }}" width="26" height="22"
                                         border="0" alt="SEARCH"/>
                        <select name="cat_id2">
                        ...
                        <input name="button" type="button" class="button"
                               onclick="searchGoods('cat_id2', 'brand_id2', 'keyword2')"
                ...
                <tr>
                    <td width="42%"><select name="source_select" id="source_select" size="20"
                                            style="width:100%;height:300px;" ondblclick="addItem(this)">
                        </select></td>
                    <td align="center">
                        <p>
                            <input name="button" type="button" class="button"
                                   onclick="addAllItem(document.getElementById('source_select'))" value="&gt;&gt;"/>
                        </p>
                        <p>
                            <input name="button" type="button" class="button"
                                   onclick="addItem(document.getElementById('source_select'))" value="&gt;"/>
                        </p>
                        <p>
                            <input name="button" type="button" class="button"
                                   onclick="removeItem(document.getElementById('target_select'))" value="&lt;"/>
                        </p>
                        <p>
                            <input name="button" type="button" class="button" value="&lt;&lt;"
                                   onclick="removeItem(document.getElementById('target_select'), true)"/>
                        </p>
                    </td>
                    <td width="42%"><select name="target_select" id="target_select" size="20"
                                            style="width:100%;height:300px" multiple="multiple">
                        </select></td>
                </tr>
            </table>
            <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" id="desc-table"
                   style="display:none;">
                <tr>
                    <td>{{ $FCKeditor }}</td>
                </tr>
            </table>
            <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" id="advanced-table"
                   style="display:none;">
                <tr>
                    <td class="label"><a href="javascript:showNotice('noticeTemplateFile');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}"
                                width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a>{{ $lang['template_file'] }}</td>
                    <td><input name="topic_template_file" type="text" id="topic_template_file"
                               value="{{ $topic['template'] }}" size="40"/>
                        <span class="notice-span" {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }}
                        id="noticeTemplateFile">{{ $lang['notice_template_file'] }}</span>
                    </td>
                </tr>
                <tr>
                    <td class="label"><a href="javascript:showNotice('noticeCss');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}"
                                width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a>{{ $lang['style_sheet'] }}</td>
                    <td><textarea name="topic_css" id="topic_css" cols="40" rows="5">{{ $topic['css'] }}</textarea>
                        <span class="notice-span" {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }}
                        id="noticeCss">{{ $lang['notice_css'] }}</span>
                        <div><a href="javascript:chanageSize(3,'topic_css');">[+]</a> <a
                                href="javascript:chanageSize(-3,'topic_css');">[-]</a></div>
                    </td>
                </tr>
            </table>
