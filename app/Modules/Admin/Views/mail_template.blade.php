            ...
            <table id="general-table" align="center">
                ...
                <tr>
                    ...
                    <td><input type="text" name="subject" id="subject" style="width: 300px"
                               value="{{ $template['template_subject'] }}"/></td>
                </tr>
                <tr>
                    <td style="font-weight: bold">{{ $lang['mail_type'] }}:</td>
                    <td>
                        <input type="radio" name="mail_type" value="0" @if($template['is_html'] === '0')checked="true"
                               @endif onclick="javascript:change_editor();"/>{{ $lang['mail_plain_text'] }}
                        <input type="radio" name="mail_type" value="1" @if($template['is_html'] === '1')checked="true"
                               @endif onclick="javascript:change_editor();"/>{{ $lang['mail_html'] }}
                ...
                <tr>
                    <td colspan="2">@if($template['is_html'] === '1')
                            {{ $FCKeditor }}
                        @else
                            <textarea name="content" id="content" style="width:90%"
                                      rows="16">{{ $template['template_content'] }}</textarea>
                        @endif</td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><input type="submit" value="{{ $lang['button_submit'] }}"
                                                          class="button"/></td>
                </tr>
            </table>
        </form>
