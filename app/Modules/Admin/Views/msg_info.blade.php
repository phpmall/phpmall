    ...
    <table width="98%">
        <tr>
            <td style="padding: 0px 20px">
                ...
                @endif
                <div align="right" nowrap="nowrap">【 '.($msg['msg_area'] === "1" ? '{{ $lang['message_board'] }}' :
                    '{{ $lang['user_center'] }}').' 】<a
                        href="mailto:{{ $msg['user_email'] }}">{{ $msg['user_name'] }}</a> @ {{ $msg['msg_time'] }}
                </div>
            </td>
        </tr>
        @if($msg['msg_area'] === "1")
            <tr>
                <td align="center">
                    @if($msg['msg_status'] === "0")
                        <input type="button"
                               onclick="location.href='user_msg.php?act=check&check=allow&id={{ $msg['msg_id'] }}'"
                               value="{{ $lang['allow'] }}" class="button"/>
                    @else
                        <input type="button"
                               onclick="location.href='user_msg.php?act=check&check=forbid&id={{ $msg['msg_id'] }}'"
    ...
    <form method="post" action="user_msg.php?act=action" name="theForm" onsubmit="return validate()">
        <table border="0" width="98%">
            ...
            <tr>
                ...
                <td rowspan="2"><textarea name="msg_content" cols="50" rows="4" wrap="VIRTUAL"
