...
<div class="form-div">
    <form action="javascript:searchGoods()" name="searchForm">
        ...
        <!-- 分类 -->
        <select name="cat_id">
            <option value="0">{{ $lang['all_cat'] }}</caption>{{ $cat_list }}</select>
        <!-- 品牌 -->
    ...
    <div class="main-div">
        <table id="group-table" cellspacing="1" cellpadding="3" width="100%">
            ...
            <tr>
                ...
                <td>
                    <input name="start_time" type="text" id="start_time" size="22"
                           value='{{ $group_buy['start_time'] }}' readonly="readonly"/><input name="selbtn1"
                                                                                              type="button" id="selbtn1"
            ...
            <tr>
                ...
                <td>
                    <input name="end_time" type="text" id="end_time" size="22" value='{{ $group_buy['end_time'] }}'
            ...
            <tr>
                ...
                <td><input name="deposit" type="text" id="deposit" value="{{ $group_buy['deposit'] ?? 0 }}" size="30">
                </td>
            </tr>
            <tr>
                <td class="label"><a href="javascript:showNotice('noticRestrict');"
                                     title="{{ $lang['form_notice'] }}"><img
                            src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                            alt="{{ $lang['form_notice'] }}"></a>{{ $lang['label_restrict_amount'] }}</td>
                <td><input type="text" name="restrict_amount" value="{{ $group_buy['restrict_amount'] ?? 0 }}"
                           size="30"/>
                    <br/><span class="notice-span"
                               {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticRestrict">{{ $lang['notice_restrict_amount'] }}</span>
                </td>
            </tr>
            <tr>
                <td class="label">{{ $lang['label_gift_integral'] }}</td>
                <td><input type="text" name="gift_integral" value="{{ $group_buy['gift_integral'] ?? 0 }}" size="30"/>
                </td>
            </tr>
            @foreach($group_buy['price_ladder'] as $key => $item)
                @if($key === 0)
                    <tr>
                        ...
                        <td>{{ $lang['notice_ladder_amount'] }} <input type="text" name="ladder_amount[]"
                                                                       value="{{ $item['amount'] }}" size="8"/>&nbsp;&nbsp;
                            {{ $lang['notice_ladder_price'] }} <input type="text" name="ladder_price[]"
                                                                      value="{{ $item['price'] }}" size="8"/>
            ...
            <tr>
                ...
                <td>
                    ...
                    @if($group_buy['status'] === 1)
                        <input type="submit" name="finish" value="{{ $lang['button_finish'] }}" class="button"
                               onclick="return confirm('{{ $lang['notice_finish'] }}')"/>
                    @elseif($group_buy['status'] === 2)
                        <input type="submit" name="succeed" value="{{ $lang['button_succeed'] }}" class="button"
                               onclick="return confirm(succeed_confirm)"/>{{ $lang['notice_succeed'] }}<br/>
                        <input type="submit" name="fail" value="{{ $lang['button_fail'] }}" class="button"
                               onclick="return confirm(fail_confirm)"/>{{ $lang['notice_fail'] }}
                    @elseif($group_buy['status'] === 3)
                        <input type="submit" name="mail" value="{{ $lang['button_mail'] }}" class="button"
                               onclick="return confirm('{{ $lang['notice_mail'] }}')"/>
                    @endif    </td>
            </tr>
        </table>
    </div>
