...
<script src="{{ asset('static/admin/js/colorselector.js') }}"></script>
    ...
    <div id="tabbar-div">
        <p>
            <span class="tab-front" id="general-tab">{{ $lang['tab_general'] }}</span><span
                ...
                class="tab-back" id="mix-tab">{{ $lang['tab_mix'] }}</span>@if($goods_type_list)
                <span
                    class="tab-back" id="properties-tab">{{ $lang['tab_properties'] }}</span>
            @endif<span
                class="tab-back" id="gallery-tab">{{ $lang['tab_gallery'] }}</span><span
                class="tab-back" id="linkgoods-tab">{{ $lang['tab_linkgoods'] }}</span>@if($code === '')
                <span
                    class="tab-back" id="groupgoods-tab">{{ $lang['tab_groupgoods'] }}</span>
            @endif<span
        ...
        <form enctype="multipart/form-data" action="" method="post" name="theForm">
            ...
            <table width="90%" id="general-table" align="center">
                <tr>
                    ...
                    <td><input type="text" name="goods_name" value="{{ $goods['goods_name'] }}"
                        ...
                        <div style="background-color:{{ $goods_name_color }};float:left;margin-left:2px;"
                             id="font_color" onclick="ColorSelecter.Show(this);"><img
                                src="{{ asset('static/admin/images/color_selecter.gif') }}" style="margin-top:-1px;"/>
                        </div>
                        <input type="hidden" id="goods_name_color" name="goods_name_color"
                               value="{{ $goods_name_color }}"/>&nbsp;
                ...
                <tr>
                    <td class="label">
                        <a href="javascript:showNotice('noticeGoodsSN');" title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a> {{ $lang['lab_goods_sn'] }} </td>
                    <td><input type="text" name="goods_sn" value="{{ $goods['goods_sn'] }}" size="20"
                               onblur="checkGoodsSn(this.value,'{{ $goods['goods_id'] }}')"/><span
                            id="goods_sn_notice"></span><br/>
                        <span class="notice-span"
                              {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeGoodsSN">{{ $lang['notice_goods_sn'] }}</span>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><select name="cat_id" onchange="hideCatDiv()">
                        ...
                        @if($is_add)
                            <a href="javascript:void(0)" onclick="rapidCatAdd()" title="{{ $lang['rapid_add_cat'] }}"
                               class="special">{{ $lang['rapid_add_cat'] }}</a>
                            <span id="category_add" style="display:none;">
              <input class="text" size="10" name="addedCategoryName"/>
               <a href="javascript:void(0)" onclick="addCategory()" title="{{ $lang['button_submit'] }}"
                  class="special">{{ $lang['button_submit'] }}</a>
               <a href="javascript:void(0)" onclick="return goCatPage()" title="{{ $lang['category_manage'] }}"
                  class="special">{{ $lang['category_manage'] }}</a>
               <a href="javascript:void(0)" onclick="hideCatDiv()" title="{{ $lang['hide'] }}" class="special"><<</a>
                ...
                <tr>
                    ...
                    <td>
                        <input type="button" value="{{ $lang['add'] }}" onclick="addOtherCat(this.parentNode)"
                               class="button"/>
                        @foreach($goods['other_cat'] as $cat_id)
                            <select name="other_cat[]">
                                <option value="0">{{ $lang['select_please'] }}</option>
                                {$other_cat_list.$cat_id}</select>
                        @endforeach
                    </td>
                </tr>
                <tr>
                    ...
                    <td><select name="brand_id" onchange="hideBrandDiv()">
                            <option value="0">{{ $lang['select_please'] }}{html_options options=$brand_list
                                selected=$goods.brand_id}
                        </select>
                        @if($is_add)
                            <a href="javascript:void(0)" title="{{ $lang['rapid_add_brand'] }}"
                               onclick="rapidBrandAdd()" class="special">{{ $lang['rapid_add_brand'] }}</a>
              ...
              <input class="text" size="15" name="addedBrandName"/>
               ...
               <a href="javascript:void(0)" onclick="return goBrandPage()" title="{{ $lang['brand_manage'] }}"
                  class="special">{{ $lang['brand_manage'] }}</a>
               <a href="javascript:void(0)" onclick="hideBrandDiv()" title="{{ $lang['hide'] }}" class="special"><<</a>
                ...
                @if($user_rank_list)
                    <tr>
                        <td class="label"><a href="javascript:showNotice('noticeUserPrice');"
                                             title="{{ $lang['form_notice'] }}"><img
                                    src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16"
                                    border="0" alt="{{ $lang['form_notice'] }}"></a>{{ $lang['lab_user_price'] }}</td>
                        <td>
                            @foreach($user_rank_list as $user_rank)
                                {{ $user_rank['rank_name'] }}<span id="nrank_{{ $user_rank['rank_id'] }}"></span><input
                                    type="text" id="rank_{{ $user_rank['rank_id'] }}" name="user_price[]"
                                    value="{$member_price_list[$user_rank.rank_id]|default:-1}"
                                    onkeyup="if(parseInt(this.value)<-1){this.value='-1';};set_price_note({{ $user_rank['rank_id'] }})"
                                    size="8"/>
                                <input type="hidden" name="user_rank[]" value="{{ $user_rank['rank_id'] }}"/>
                            @endforeach
                            <br/>
                            <span class="notice-span"
                                  {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeUserPrice">{{ $lang['notice_user_price'] }}</span>
                        </td>
                    </tr>
                @endif
                ...
                <!--鍟嗗搧浼樻儬浠锋牸-->
                <tr>
                    <td class="label"><a href="javascript:showNotice('volumePrice');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a>{{ $lang['lab_volume_price'] }}</td>
                    <td>
                        <table width="100%" id="tbody-volume" align="center">
                            @foreach($volume_price_list as $volume_price)
                                <tr>
                                    <td>
                                        @if($loop->volume_price_tab.iteration === 1)
                                            <a href="javascript:;" onclick="addVolumePrice(this)">[+]</a>
                                        @else
                                            <a href="javascript:;" onclick="removeVolumePrice(this)">[-]</a>
                                        @endif
                                        {{ $lang['volume_number'] }} <input type="text" name="volume_number[]" size="8"
                                                                            value="{{ $volume_price['number'] }}"/>
                                        {{ $lang['volume_price'] }} <input type="text" name="volume_price[]" size="8"
                                                                           value="{{ $volume_price['price'] }}"/>
                                    </td>
                                </tr>
                            @endforeach
                        </table>
                        <span class="notice-span"
                              {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="volumePrice">{{ $lang['notice_volume_price'] }}</span>
                    </td>
                </tr>
                <!--鍟嗗搧浼樻儬浠锋牸 end -->
                ...
                <tr>
                    ...
                    <td><input type="text" name="market_price" value="{{ $goods['market_price'] }}" size="20"/>
                        <input type="button" value="{{ $lang['integral_market_price'] }}"
                               onclick="integral_market_price()"/>
                    </td>
                </tr>
                <tr>
                    <td class="label"><a href="javascript:showNotice('giveIntegral');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a> {{ $lang['lab_give_integral'] }}</td>
                    <td><input type="text" name="give_integral" value="{{ $goods['give_integral'] }}" size="20"/>
                        <br/><span class="notice-span"
                                   {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="giveIntegral">{{ $lang['notice_give_integral'] }}</span>
                    </td>
                </tr>
                <tr>
                    <td class="label"><a href="javascript:showNotice('rankIntegral');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a> {{ $lang['lab_rank_integral'] }}</td>
                    <td><input type="text" name="rank_integral" value="{{ $goods['rank_integral'] }}" size="20"/>
                        <br/><span class="notice-span"
                                   {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="rankIntegral">{{ $lang['notice_rank_integral'] }}</span>
                    </td>
                </tr>
                <tr>
                    <td class="label"><a href="javascript:showNotice('noticPoints');"
                                         title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a> {{ $lang['lab_integral'] }}</td>
                    <td><input type="text" name="integral" value="{{ $goods['integral'] }}" size="20"
                               onblur="parseint_integral()" ;/>
                        <br/><span class="notice-span"
                                   {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticPoints">{{ $lang['notice_integral'] }}</span>
                    </td>
                </tr>
                <tr>
                    <td class="label"><label for="is_promote"><input type="checkbox" id="is_promote" name="is_promote"
                                                                     value="1"
                                                                     @if($goods['is_promote'])checked="checked"
                                                                     @endif onclick="handlePromote(this.checked);"/> {{ $lang['lab_promote_price'] }}
                        </label></td>
                    <td id="promote_3"><input type="text" id="promote_1" name="promote_price"
                                              value="{{ $goods['promote_price'] }}" size="20"/></td>
                </tr>
                <tr id="promote_4">
                    <td class="label" id="promote_5">{{ $lang['lab_promote_date'] }}</td>
                    <td id="promote_6">
                        <input name="promote_start_date" type="text" id="promote_start_date" size="12"
                               value='{{ $goods['promote_start_date'] }}' readonly="readonly"/><input name="selbtn1"
                                                                                                      type="button"
                                                                                                      id="selbtn1"
                                                                                                      onclick="return showCalendar('promote_start_date', '%Y-%m-%d', false, false, 'selbtn1');"
                                                                                                      value="{{ $lang['btn_select'] }}"
                                                                                                      class="button"/> -
                        <input name="promote_end_date" type="text" id="promote_end_date" size="12"
                               value='{{ $goods['promote_end_date'] }}' readonly="readonly"/><input name="selbtn2"
                                                                                                    type="button"
                                                                                                    id="selbtn2"
                                                                                                    onclick="return showCalendar('promote_end_date', '%Y-%m-%d', false, false, 'selbtn2');"
                                                                                                    value="{{ $lang['btn_select'] }}"
                                                                                                    class="button"/>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        @if($goods['goods_img'])
                            <a href="goods.php?act=show_image&img_url={{ $goods['goods_img'] }}" target="_blank"><img
                                    src="{{ asset('static/admin/images/yes.gif') }}" border="0"/></a>
                        @else
                            <img src="{{ asset('static/admin/images/no.gif') }}"/>
                        @endif
                        <br/><input type="text" size="40" value="{{ $lang['lab_picture_url'] }}" style="color:#aaa;"
                                    onfocus="if (this.value === '{{ $lang['lab_picture_url'] }}'){this.value='http://';this.style.color='#000';}"
                                    name="goods_img_url"/>
                    </td>
                </tr>
                <tr id="auto_thumb_1">
                    ...
                    <td id="auto_thumb_3">
                        ...
                        @if($goods['goods_thumb'])
                            <a href="goods.php?act=show_image&img_url={{ $goods['goods_thumb'] }}" target="_blank"><img
                                    src="{{ asset('static/admin/images/yes.gif') }}" border="0"/></a>
                        @else
                            <img src="{{ asset('static/admin/images/no.gif') }}"/>
                        @endif
                        <br/><input type="text" size="40" value="{{ $lang['lab_thumb_url'] }}" style="color:#aaa;"
                                    onfocus="if (this.value === '{{ $lang['lab_thumb_url'] }}'){this.value='http://';this.style.color='#000';}"
                                    name="goods_thumb_url"/>
                        @if($gd > 0)
                            <br/><label for="auto_thumb"><input type="checkbox" id="auto_thumb" name="auto_thumb"
                                                                checked="true" value="1"
                                                                onclick="handleAutoThumb(this.checked)"/>{{ $lang['auto_thumb'] }}
                            </label>
                        @endif
                    </td>
                </tr>
            ...
            <table width="90%" id="mix-table" style="display:none" align="center">
                @if($code === '')
                    <tr>
                        ...
                        <td><input type="text" name="goods_weight" value="{{ $goods['goods_weight_by_unit'] }}"
                                   size="20"/> <select name="weight_unit">{html_options options=$unit_list
                                selected=$weight_unit}</select></td>
                    </tr>
                @endif
                @if($cfg['use_storage'])
                    <tr>
                        <td class="label"><a href="javascript:showNotice('noticeStorage');"
                                             title="{{ $lang['form_notice'] }}"><img
                                    src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16"
                                    border="0" alt="{{ $lang['form_notice'] }}"></a> {{ $lang['lab_goods_number'] }}
                        </td>
                        <!--            <td><input type="text" name="goods_number" value="{{ $goods['goods_number'] }}" size="20" @if($code != '' || $goods['_attribute'] != '')
                            readonly="readonly"

                        @endif /><br />-->
                        <td><input type="text" name="goods_number" value="{{ $goods['goods_number'] }}" size="20"/><br/>
                            <span class="notice-span"
                                  {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeStorage">{{ $lang['notice_storage'] }}</span>
                        </td>
                    </tr>
                    <tr>
                ...
                <tr>
                    ...
                    <td><input type="checkbox" name="is_best" value="1"
                               @if($goods['is_best'])checked="checked"@endif />{{ $lang['is_best'] }} <input
                            type="checkbox" name="is_new" value="1"
                            @if($goods['is_new'])checked="checked"@endif />{{ $lang['is_new'] }} <input type="checkbox"
                                                                                                        name="is_hot"
                                                                                                        value="1"
                                                                                                        @if($goods['is_hot'])checked="checked"@endif />{{ $lang['is_hot'] }}
                    </td>
                </tr>
                <tr id="alone_sale_1">
                    <td class="label" id="alone_sale_2">{{ $lang['lab_is_on_sale'] }}</td>
                    <td id="alone_sale_3"><input type="checkbox" name="is_on_sale" value="1"
                                                 @if($goods['is_on_sale'])checked="checked"@endif /> {{ $lang['on_sale_desc'] }}
                    </td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['lab_is_alone_sale'] }}</td>
                    <td><input type="checkbox" name="is_alone_sale" value="1"
                               @if($goods['is_alone_sale'])checked="checked"@endif /> {{ $lang['alone_sale'] }}</td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['lab_is_free_shipping'] }}</td>
                    <td><input type="checkbox" name="is_shipping" value="1"
                               @if($goods['is_shipping'])checked="checked"@endif /> {{ $lang['free_shipping'] }}</td>
                </tr>
                <tr>
                    <td class="label">{{ $lang['lab_keywords'] }}</td>
                    <td><input type="text" name="keywords" value="{{ $goods['keywords'] }}"
                               size="40"/> {{ $lang['notice_keywords'] }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class="label">
                        <a href="javascript:showNotice('noticeSellerNote');" title="{{ $lang['form_notice'] }}"><img
                                src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16" border="0"
                                alt="{{ $lang['form_notice'] }}"></a> {{ $lang['lab_seller_note'] }} </td>
                    <td><textarea name="seller_note" cols="40" rows="3">{{ $goods['seller_note'] }}</textarea><br/>
                        <span class="notice-span"
                              {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeSellerNote">{{ $lang['notice_seller_note'] }}</span>
                    </td>
                </tr>
            </table>
            ...
            @if($goods_type_list)
                <table width="90%" id="properties-table" style="display:none" align="center">
                    <tr>
                        <td class="label"><a href="javascript:showNotice('noticeGoodsType');"
                                             title="{{ $lang['form_notice'] }}"><img
                                    src="{{ asset('static/admin/images/notice.gif') }}" width="16" height="16"
                                    border="0" alt="{{ $lang['form_notice'] }}"></a>{{ $lang['lab_goods_type'] }}</td>
                        <td>
                            ...
                            <span class="notice-span"
                                  {{ $help_open ? 'style="display:block" ' : ' style="display:none" ' }} id="noticeGoodsType">{{ $lang['notice_goods_type'] }}</span>
                        </td>
                    </tr>
                    <tr>
            ...
            <table width="90%" id="gallery-table" style="display:none" align="center">
                ...
                <tr>
                    <td>
                        @foreach($img_list as $img => $i)
                            <div id="gallery_{{ $img['img_id'] }}"
                                 style="float:left; text-align:center; border: 1px solid #DADADA; margin: 4px; padding:2px;">
                                <a href="javascript:;"
                                   onclick="if (confirm('{{ $lang['drop_img_confirm'] }}')) dropImg('{{ $img['img_id'] }}')">[-]</a><br/>
                                <a href="goods.php?act=show_image&img_url={{ $img['img_url'] }}" target="_blank">
                                    <img
                                        src="../'.($img['thumb_url'] ? '{{ $img['thumb_url'] }}' : '{{ $img['img_url'] }}').'"
                                        @if($thumb_width != 0)width="{{ $thumb_width }}"
                                        @endif @if($thumb_height != 0)height="{{ $thumb_height }}" @endif border="0"/>
                                </a><br/>
                                <input type="text" value="{{ $img['img_desc'] }}" size="15"
                                       name="old_img_desc[{{ $img['img_id'] }}]"/>
                            </div>
                        @endforeach
                    </td>
                </tr>
                ...
                <tr>
                    <td>
                        ...
                        <input type="text" size="40" value="{{ $lang['img_file'] }}" style="color:#aaa;"
                               onfocus="if (this.value === '{{ $lang['img_file'] }}'){this.value='http://';this.style.color='#000';}"
                               name="img_file[]"/>
                    </td>
                </tr>
            </table>
            ...
            <table width="90%" id="linkgoods-table" style="display:none" align="center">
                ...
                <tr>
                    <td colspan="3">
                        <img src="{{ asset('static/admin/images/icon_search.gif') }}" width="26" height="22" border="0"
                             alt="SEARCH"/>
                        <select name="cat_id1">
                            <option value="0">{{ $lang['all_category'] }}{{ $cat_list }}</select>
                ...
                <tr>
                    <td width="42%">
                        <select name="source_select1" size="20" style="width:100%"
                                ondblclick="sz1.addItem(false, 'add_link_goods', goodsId, this.form.elements['is_single'][0].checked)"
                                multiple="true">
                        </select>
                    </td>
                    <td align="center">
                        <p><input name="is_single" type="radio" value="1" checked="checked"/>{{ $lang['single'] }}
                            <br/><input name="is_single" type="radio" value="0"/>{{ $lang['double'] }}</p>
                        <p><input type="button" value=">>"
                                  onclick="sz1.addItem(true, 'add_link_goods', goodsId, this.form.elements['is_single'][0].checked)"
                                  class="button"/></p>
                        <p><input type="button" value=">"
                                  onclick="sz1.addItem(false, 'add_link_goods', goodsId, this.form.elements['is_single'][0].checked)"
                                  class="button"/></p>
                        <p><input type="button" value="<"
                                  onclick="sz1.dropItem(false, 'drop_link_goods', goodsId, elements['is_single'][0].checked)"
                                  class="button"/></p>
                        <p><input type="button" value="<<"
                                  onclick="sz1.dropItem(true, 'drop_link_goods', goodsId, elements['is_single'][0].checked)"
                                  class="button"/></p>
                    </td>
                    <td width="42%">
                        <select name="target_select1" size="20" style="width:100%" multiple
                                ondblclick="sz1.dropItem(false, 'drop_link_goods', goodsId, elements['is_single'][0].checked)">
            ...
            <table width="90%" id="groupgoods-table" style="display:none" align="center">
                ...
                <tr>
                    <td colspan="3">
                        ...
                        <input type="button" value="{{ $lang['button_search'] }}"
                               onclick="searchGoods(sz2, 'cat_id2', 'brand_id2', 'keyword2')" class="button"/>
                    </td>
                </tr>
                <!-- 鍟嗗搧鍒楄〃 -->
                <tr>
                ...
                <tr>
                    <td width="42%">
                        <select name="source_select2" size="20" style="width:100%"
                                onchange="sz2.priceObj.value = this.options[this.selectedIndex].id"
                                ondblclick="sz2.addItem(false, 'add_group_goods', goodsId, this.form.elements['price2'].value)">
                        </select>
                    </td>
                    <td align="center">
                        <p>{{ $lang['price'] }}<br/><input name="price2" type="text" size="6"/></p>
                        <p><input type="button" value=">"
                                  onclick="sz2.addItem(false, 'add_group_goods', goodsId, this.form.elements['price2'].value)"
                                  class="button"/></p>
                        <p><input type="button" value="<"
                                  onclick="sz2.dropItem(false, 'drop_group_goods', goodsId, elements['is_single'][0].checked)"
                                  class="button"/></p>
                        <p><input type="button" value="<<"
                                  onclick="sz2.dropItem(true, 'drop_group_goods', goodsId, elements['is_single'][0].checked)"
                                  class="button"/></p>
                    </td>
                    <td width="42%">
                        <select name="target_select2" size="20" style="width:100%" multiple
                                ondblclick="sz2.dropItem(false, 'drop_group_goods', goodsId, elements['is_single'][0].checked)">
            ...
            <table width="90%" id="article-table" style="display:none" align="center">
                ...
                <tr>
                    <td colspan="3">
                        ...
                        <input type="button" value="{{ $lang['button_search'] }}" onclick="searchArticle()"
                               class="button"/>
                    </td>
                </tr>
                <!-- 鏂囩珷鍒楄〃 -->
                ...
                <tr>
                    <td width="45%">
                        <select name="source_select3" size="20" style="width:100%" multiple
                                ondblclick="sz3.addItem(false, 'add_goods_article', goodsId, this.form.elements['price2'].value)">
                        </select>
                    </td>
                    <td align="center">
                        <p><input type="button" value=">>"
                                  onclick="sz3.addItem(true, 'add_goods_article', goodsId, this.form.elements['price2'].value)"
                                  class="button"/></p>
                        <p><input type="button" value=">"
                                  onclick="sz3.addItem(false, 'add_goods_article', goodsId, this.form.elements['price2'].value)"
                                  class="button"/></p>
                        <p><input type="button" value="<"
                                  onclick="sz3.dropItem(false, 'drop_goods_article', goodsId, elements['is_single'][0].checked)"
                                  class="button"/></p>
                        <p><input type="button" value="<<"
                                  onclick="sz3.dropItem(true, 'drop_goods_article', goodsId, elements['is_single'][0].checked)"
                                  class="button"/></p>
                    </td>
                    <td width="45%">
                        <select name="target_select3" size="20" style="width:100%" multiple
                                ondblclick="sz3.dropItem(false, 'drop_goods_article', goodsId, elements['is_single'][0].checked)">
                            @foreach($goods_article_list as $goods_article)
                                <option
                                    value="{{ $goods_article['article_id'] }}">{{ $goods_article['title'] }}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
            ...
            <div class="button-div">
                ...
                @endif
                <input type="button" value="{{ $lang['button_submit'] }}" class="button"
                       onclick="validate('{{ $goods['goods_id'] }}')"/>
