        ...
        @foreach($msg_list as $msg)
            <div style="margin-bottom:10px;border:1px solid #ccc;background:white;">
                <div style="border-bottom: 1px solid #eee;position:relative;">
                    <strong>{$msg.msg_title|escape:"html"}</strong><a
                        href="user_msg.php?act=remove_msg&order_id={{ $order_id }}&user_id={{ $user_id }}&msg_id={{ $msg['msg_id'] }}"
                        style="position:absolute ;right:5px"
                        onclick="if (!confirm('{{ $lang['confirm_delete'] }}')) return false;"><img
                            src="{{ asset('static/admin/images/icon_drop.gif') }}" border="0"/></a></div>
                <div>{$msg.msg_content|escape:"html"|nl2br}</div>
                @if($msg['message_img'])
                    <div align="right">
                        <a href="../data/feedbackimg/{{ $msg['message_img'] }}" target="_bank" width="300"
                           height="400">{{ $lang['view_upload_file'] }}</a>
                        <a
                            href="user_msg.php?act=drop_file&id={{ $msg['msg_id'] }}&file={{ $msg['message_img'] }}">{{ $lang['drop'] }}</a>
                    </div>
                @endif
                <div align="right" nowrap="nowrap"><a href="mailto:{{ $msg['user_email'] }}">{{ $msg['user_name'] }}</a>
                    @
                    {{ $msg['msg_time'] }}</div>
            </div>
        @endforeach
        ...
        <table border="0" width="100%">
            ...
            <tr>
                ...
                <td><textarea name="msg_content" cols="50" rows="4" wrap="VIRTUAL"
                              id="msg_content">{{ $msg['reply_content'] }}</textarea></td>
            </tr>
            <tr>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
