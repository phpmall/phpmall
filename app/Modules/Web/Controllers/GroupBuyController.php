<?php

declare(strict_types=1);

namespace App\Modules\Web\Controllers;

use App\Helpers\CommonHelper;
use App\Helpers\GoodsHelper;
use App\Helpers\MainHelper;
use App\Helpers\OrderHelper;
use App\Helpers\TimeHelper;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Session;

class GroupBuyController extends BaseController
{
    public function index(Request $request)
    {
        $action = $request->get('act');
        /**
         * 团购商品 --> 团购活动商品列表
         */
        if ($action === 'list') {
            // 取得团购活动总数
            $count = $this->group_buy_count();
            if ($count > 0) {
                // 取得每页记录数
                $size = cfg('page_size') && intval(cfg('page_size')) > 0 ? intval(cfg('page_size')) : 10;

                // 计算总页数
                $page_count = ceil($count / $size);

                // 取得当前页
                $page = isset($_REQUEST['page']) && intval($_REQUEST['page']) > 0 ? intval($_REQUEST['page']) : 1;
                $page = $page > $page_count ? $page_count : $page;

                // 缓存id：语言 - 每页记录数 - 当前页
                $cache_id = cfg('lang').'-'.$size.'-'.$page;
                $cache_id = sprintf('%X', crc32($cache_id));
            } else {
                // 缓存id：语言
                $cache_id = cfg('lang');
                $cache_id = sprintf('%X', crc32($cache_id));
            }

            // 如果没有缓存，生成缓存
            if (! $this->is_cached('group_buy_list', $cache_id)) {
                if ($count > 0) {
                    // 取得当前页的团购活动
                    $gb_list = $this->group_buy_list($size, $page);
                    $this->assign('gb_list', $gb_list);

                    // 设置分页链接
                    $pager = MainHelper::get_pager('group_buy.php', ['act' => 'list'], $count, $page, $size);
                    $this->assign('pager', $pager);
                }

                $this->assign('cfg', cfg());
                $this->assign_template();
                $position = $this->assign_ur_here();
                $this->assign('page_title', $position['title']);    // 页面标题
                $this->assign('ur_here', $position['ur_here']);  // 当前位置
                $this->assign('categories', GoodsHelper::get_categories_tree()); // 分类树
                $this->assign('helps', MainHelper::get_shop_help());       // 网店帮助
                $this->assign('top_goods', GoodsHelper::get_top10());           // 销售排行
                $this->assign('promotion_info', CommonHelper::get_promotion_info());
                $this->assign('feed_url', (cfg('rewrite') === 1) ? 'feed-typegroup_buy.xml' : 'feed.php?type=group_buy'); // RSS URL

                $this->assign_dynamic('group_buy_list');
            }

            return $this->display('group_buy_list', $cache_id);
        }

        /**
         * 团购商品 --> 商品详情
         */
        if ($action === 'view') {
            // 取得参数：团购活动id
            $group_buy_id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
            if ($group_buy_id <= 0) {
                return response()->redirectTo('/');
            }

            // 取得团购活动信息
            $group_buy = GoodsHelper::group_buy_info($group_buy_id);

            if (empty($group_buy)) {
                return response()->redirectTo('/');
            }
            //    elseif ($group_buy['is_on_sale'] === 0 || $group_buy['is_alone_sale'] === 0)
            //    {
            //        header("Location: ./\n");
            //        exit;
            //    }

            // 缓存id：语言，团购活动id，状态，（如果是进行中）当前数量和是否登录
            $cache_id = cfg('lang').'-'.$group_buy_id.'-'.$group_buy['status'];
            if ($group_buy['status'] === GBS_UNDER_WAY) {
                $cache_id = $cache_id.'-'.$group_buy['valid_goods'].'-'.intval(Session::get('user_id') > 0);
            }
            $cache_id = sprintf('%X', crc32($cache_id));

            // 如果没有缓存，生成缓存
            if (! $this->is_cached('group_buy_goods', $cache_id)) {
                $group_buy['gmt_end_date'] = $group_buy['end_date'];
                $this->assign('group_buy', $group_buy);

                // 取得团购商品信息
                $goods_id = $group_buy['goods_id'];
                $goods = GoodsHelper::goods_info($goods_id);
                if (empty($goods)) {
                    return response()->redirectTo('/');
                }
                $goods['url'] = build_uri('goods', ['gid' => $goods_id], $goods['goods_name']);
                $this->assign('gb_goods', $goods);

                // 取得商品的规格
                $properties = GoodsHelper::get_goods_properties($goods_id);
                $this->assign('specification', $properties['spe']); // 商品规格

                // 模板赋值
                $this->assign('cfg', cfg());
                $this->assign_template();

                $position = $this->assign_ur_here(0, $goods['goods_name']);
                $this->assign('page_title', $position['title']);    // 页面标题
                $this->assign('ur_here', $position['ur_here']);  // 当前位置

                $this->assign('categories', GoodsHelper::get_categories_tree()); // 分类树
                $this->assign('helps', MainHelper::get_shop_help());       // 网店帮助
                $this->assign('top_goods', GoodsHelper::get_top10());           // 销售排行
                $this->assign('promotion_info', CommonHelper::get_promotion_info());
                $this->assign_dynamic('group_buy_goods');
            }

            // 更新商品点击次数
            DB::table('goods')
                ->where('goods_id', $group_buy['goods_id'])
                ->increment('click_count');

            $this->assign('now_time', TimeHelper::gmtime());           // 当前系统时间

            return $this->display('group_buy_goods', $cache_id);
        }

        /**
         * 团购商品 --> 购买
         */
        if ($action === 'buy') {
            // 查询：判断是否登录
            if (Session::get('user_id') <= 0) {
                $this->show_message(lang('gb_error_login'), '', '', 'error');
            }

            // 查询：取得参数：团购活动id
            $group_buy_id = isset($_POST['group_buy_id']) ? intval($_POST['group_buy_id']) : 0;
            if ($group_buy_id <= 0) {
                return response()->redirectTo('/');
            }

            // 查询：取得数量
            $number = isset($_POST['number']) ? intval($_POST['number']) : 1;
            $number = $number < 1 ? 1 : $number;

            // 查询：取得团购活动信息
            $group_buy = GoodsHelper::group_buy_info($group_buy_id, $number);
            if (empty($group_buy)) {
                return response()->redirectTo('/');
            }

            // 查询：检查团购活动是否是进行中
            if ($group_buy['status'] != GBS_UNDER_WAY) {
                $this->show_message(lang('gb_error_status'), '', '', 'error');
            }

            // 查询：取得团购商品信息
            $goods = GoodsHelper::goods_info($group_buy['goods_id']);
            if (empty($goods)) {
                return response()->redirectTo('/');
            }

            // 查询：判断数量是否足够
            if (($group_buy['restrict_amount'] > 0 && $number > ($group_buy['restrict_amount'] - $group_buy['valid_goods'])) || $number > $goods['goods_number']) {
                $this->show_message(lang('gb_error_goods_lacking'), '', '', 'error');
            }

            // 查询：取得规格
            $specs = '';
            foreach ($_POST as $key => $value) {
                if (strpos($key, 'spec_') !== false) {
                    $specs .= ','.intval($value);
                }
            }
            $specs = trim($specs, ',');

            // 查询：如果商品有规格则取规格商品信息 配件除外
            if ($specs) {
                $_specs = explode(',', $specs);
                $product_info = GoodsHelper::get_products_info($goods['goods_id'], $_specs);
            }

            empty($product_info) ? $product_info = ['product_number' => 0, 'product_id' => 0] : '';

            // 查询：判断指定规格的货品数量是否足够
            if ($specs && $number > $product_info['product_number']) {
                $this->show_message(lang('gb_error_goods_lacking'), '', '', 'error');
            }

            // 查询：查询规格名称和值，不考虑价格
            $res = DB::table('goods_attr as g')
                ->select('a.attr_name', 'g.attr_value')
                ->join('goods_type_attribute as a', 'g.attr_id', '=', 'a.attr_id')
                ->whereIn('g.goods_attr_id', explode(',', $specs))
                ->get()
                ->map(fn ($item) => (array) $item)
                ->all();
            foreach ($res as $row) {
                $attr_list[] = $row['attr_name'].': '.$row['attr_value'];
            }
            $goods_attr = implode(chr(13).chr(10), $attr_list);

            // 更新：清空购物车中所有团购商品
            OrderHelper::clear_cart(CART_GROUP_BUY_GOODS);

            // 更新：加入购物车
            $goods_price = $group_buy['deposit'] > 0 ? $group_buy['deposit'] : $group_buy['cur_price'];
            $cart = [
                'user_id' => Session::get('user_id'),
                'session_id' => SESS_ID,
                'goods_id' => $group_buy['goods_id'],
                'product_id' => $product_info['product_id'],
                'goods_sn' => addslashes($goods['goods_sn']),
                'goods_name' => addslashes($goods['goods_name']),
                'market_price' => $goods['market_price'],
                'goods_price' => $goods_price,
                'goods_number' => $number,
                'goods_attr' => addslashes($goods_attr),
                'goods_attr_id' => $specs,
                'is_real' => $goods['is_real'],
                'extension_code' => addslashes($goods['extension_code']),
                'parent_id' => 0,
                'rec_type' => CART_GROUP_BUY_GOODS,
                'is_gift' => 0,
            ];
            DB::table('user_cart')->insert($cart);

            // 更新：记录购物流程类型：团购
            Session::put('flow_type', CART_GROUP_BUY_GOODS);
            Session::put('extension_code', 'group_buy');
            Session::put('extension_id', $group_buy_id);

            // 进入收货人页面
            return response()->redirectTo('flow.php?step=consignee');
        }
    }

    // 取得团购活动总数
    private function group_buy_count()
    {
        $now = TimeHelper::gmtime();

        return DB::table('goods_activity')
            ->where('act_type', GAT_GROUP_BUY)
            ->where('start_time', '<=', $now)
            ->where('is_finished', '<', 3)
            ->count();
    }

    /**
     * 取得某页的所有团购活动
     *
     * @param  int  $size  每页记录数
     * @param  int  $page  当前页
     * @return array
     */
    private function group_buy_list($size, $page)
    {
        // 取得团购活动
        $gb_list = [];
        $now = TimeHelper::gmtime();
        $res = DB::table('goods_activity as b')
            ->select('b.*', DB::raw("IFNULL(g.goods_thumb, '') AS goods_thumb"), 'b.act_id AS group_buy_id', 'b.start_time AS start_date', 'b.end_time AS end_date')
            ->leftJoin('goods as g', 'b.goods_id', '=', 'g.goods_id')
            ->where('b.act_type', GAT_GROUP_BUY)
            ->where('b.start_time', '<=', $now)
            ->where('b.is_finished', '<', 3)
            ->orderByDesc('b.act_id')
            ->offset(($page - 1) * $size)
            ->limit($size)
            ->get()
            ->map(fn ($item) => (array) $item)
            ->all();
        foreach ($res as $group_buy) {
            $ext_info = unserialize($group_buy['ext_info']);
            $group_buy = array_merge($group_buy, $ext_info);

            // 格式化时间
            $group_buy['formated_start_date'] = TimeHelper::local_date(cfg('time_format'), $group_buy['start_date']);
            $group_buy['formated_end_date'] = TimeHelper::local_date(cfg('time_format'), $group_buy['end_date']);

            // 格式化保证金
            $group_buy['formated_deposit'] = CommonHelper::price_format($group_buy['deposit'], false);

            // 处理价格阶梯
            $price_ladder = $group_buy['price_ladder'];
            if (! is_array($price_ladder) || empty($price_ladder)) {
                $price_ladder = [['amount' => 0, 'price' => 0]];
            } else {
                foreach ($price_ladder as $key => $amount_price) {
                    $price_ladder[$key]['formated_price'] = CommonHelper::price_format($amount_price['price']);
                }
            }
            $group_buy['price_ladder'] = $price_ladder;

            // 处理图片
            if (empty($group_buy['goods_thumb'])) {
                $group_buy['goods_thumb'] = CommonHelper::get_image_path($group_buy['goods_thumb']);
            }
            // 处理链接
            $group_buy['url'] = build_uri('group_buy', ['gbid' => $group_buy['group_buy_id']]);
            // 加入数组
            $gb_list[] = $group_buy;
        }

        return $gb_list;
    }
}
