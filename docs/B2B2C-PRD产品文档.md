...
|------|----------|----------|----------|----------|
| PC 商城 | Laravel Blade | Blade 模板 + Vite + Tailwind CSS | 桌面端买家 | 搜索浏览、详情查看、下单 |
| 买家会员中心 | `packages/user` | Vue 3 + TypeScript + Vite | 已登录买家 | 订单管理、售后、地址、钱包 |
| Mobile 移动端 | `packages/mobile` | UniApp 3 (H5/小程序/App) | 移动端买家 | 分享裂变、快速下单、小程序、App |
| 商家后台 | `packages/seller` | Vue 3 + TypeScript + Vite | 商家/子账号 | 商品管理、订单处理 |
| 平台管理后台 | `packages/admin` | Vue 3 + TypeScript + Vite | 平台运营 | 商家审核、数据报表 |
| 供应商后台 | `packages/supplier` | Vue 3 + TypeScript + Vite | 供应链供应商 | 供货商品、采购订单、对账 |

### 3.2 响应式设计原则

- 移动端优先（Mobile First）
- PC 商城采用 Laravel Blade 服务端渲染，保证首屏速度和 SEO
- 买家会员中心为独立 Vue 3 SPA（`packages/user`），专注已登录用户的资产管理
- Mobile 移动端（`packages/mobile`）一套 UniApp 代码库输出 H5、微信小程序、App 三端
...
### 7.1 PC 商城：Laravel Blade + Vite + Tailwind CSS
...
### 7.3 管理后台：Vue 3 + TypeScript + Vite
