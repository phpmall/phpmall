...
PHPMall 是一款面向中大型电商场景的 **B2B2C 多商户电商平台**。平台提供基础设施和流量入口，商家入驻后拥有独立店铺，自主管理商品和订单。产品覆盖 PC 商城、H5/小程序、商家后台、平台管理后台四个终端。
...
| 终端 | 技术方案 | 目标用户 | 核心场景 |
|------|----------|----------|----------|
| PC 商城 | React 19 + Next.js 16 (SSR) | 桌面端买家 | 搜索浏览、详情查看、下单 |
| H5 商城 | UniApp 3 (H5 模式) | 移动端买家 | 分享裂变、快速下单 |
| 微信小程序 | UniApp 3 (微信模式) | 微信生态买家 | 公众号跳转、卡包 |
| 商家后台 | React 19 + Ant Design 6 | 商家/子账号 | 商品管理、订单处理 |
| 平台管理后台 | React 19 + Ant Design 6 | 平台运营 | 商家审核、数据报表 |

### 3.2 响应式设计原则

- 移动端优先（Mobile First）
- PC 商城采用 SSR 保证首屏速度和 SEO
- H5/小程序共享 UniApp 代码库，平台特化逻辑放在 `platforms/` 目录
- 商家后台和管理后台均为桌面端 Web 应用，不要求移动端适配
