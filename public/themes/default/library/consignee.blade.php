<div class="flowBox">
    ...
    <table width="99%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
            ...
            <tr>
                ...
                <td colspan="3" bgcolor="#ffffff">
                    ...
                    <select name="city" id="selCities_{{ $sn }}"
                            onchange="region.changed(this, 3, 'selDistricts_{{ $sn }}')" style="border:1px solid #ccc;">
        ...
        <tr>
            ...
            <td bgcolor="#ffffff"><input name="consignee" type="text" class="inputBg" id="consignee_{{ $sn }}"
                                         value="{{ $consignee['consignee'] }}"/>
                {{ $lang['require_field'] }} </td>
            <td bgcolor="#ffffff">{{ $lang['email_address'] }}:</td>
            <td bgcolor="#ffffff"><input name="email" type="text" class="inputBg" id="email_{{ $sn }}"
        ...
        @if($real_goods_count > 0)
            ...
            <tr>
                ...
                <td bgcolor="#ffffff"><input name="address" type="text" class="inputBg" id="address_{{ $sn }}"
                                             value="{{ $consignee['address'] }}"/>
                    {{ $lang['require_field'] }}</td>
                <td bgcolor="#ffffff">{{ $lang['postalcode'] }}:</td>
                <td bgcolor="#ffffff"><input name="zipcode" type="text" class="inputBg" id="zipcode_{{ $sn }}"
                                             value="{{ $consignee['zipcode'] }}"/></td>
            </tr>
        @endif
        <tr>
            <td bgcolor="#ffffff">{{ $lang['phone'] }}:</td>
            <td bgcolor="#ffffff"><input name="tel" type="text" class="inputBg" id="tel_{{ $sn }}"
                                         value="{{ $consignee['tel'] }}"/>
                {{ $lang['require_field'] }}</td>
            <td bgcolor="#ffffff">{{ $lang['backup_phone'] }}:</td>
            <td bgcolor="#ffffff"><input name="mobile" type="text" class="inputBg" id="mobile_{{ $sn }}"
                                         value="{{ $consignee['mobile'] }}"/></td>
        </tr>
        @if($real_goods_count > 0)
            <!-- 购物车中存在实体商品显示最佳送货时间及标志行建筑 -->
            <tr>
                <td bgcolor="#ffffff">{{ $lang['sign_building'] }}:</td>
                <td bgcolor="#ffffff"><input name="sign_building" type="text" class="inputBg"
                                             id="sign_building_{{ $sn }}" value="{{ $consignee['sign_building'] }}"/>
                </td>
                <td bgcolor="#ffffff">{{ $lang['deliver_goods_time'] }}:</td>
                <td bgcolor="#ffffff"><input name="best_time" type="text" class="inputBg" id="best_time_{{ $sn }}"
                                             value="{{ $consignee['best_time'] }}"/></td>
            </tr>
        @endif
        <tr>
            <td colspan="4" align="center" bgcolor="#ffffff">
                ...
                @if($smarty['session']['user_id'] > 0 && $consignee['address_id'] > 0)
                    ...
                    <input name="button" type="button"
                           onclick="if (confirm('{{ $lang['drop_consignee_confirm'] }}')) location.href='flow.php?step=drop_consignee&amp;id={{ $consignee['address_id'] }}'"
                           class="bnt_blue" value="{{ $lang['drop'] }}"/>
                @endif
