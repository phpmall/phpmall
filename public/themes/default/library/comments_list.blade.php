...
<div class="box">
    <div class="box_1">
        <h3><span class="text">{{ $lang['user_comment'] }}</span>({{ $lang['total'] }}<font
                class="f1">{{ $pager['record_count'] }}</font>{{ $lang['user_comment_num'] }})</h3>
        <div class="boxCenterList clearfix" style="height:1%;">
            <ul class="comments">
                @if($comments)
                    @foreach($comments as $comment)
                        <li class="word">
                            <font class="f2">'.($comment['username'] ? '{{ $comment['username'] }}' :
                                '{{ $lang['anonymous'] }}').'</font> <font class="f3">( {{ $comment['add_time'] }}
                                )</font><br/>
                ...
                <form action="javascript:;" onsubmit="submitComment(this)" method="post" name="commentForm"
                    ...
                    <table width="710" border="0" cellspacing="5" cellpadding="0">
                        <tr>
                            ...
                            <td width="631"
                            '.(!$enabled_captcha ? '@endif
                            >@if($smarty['session']['user_name']){{ $smarty['session']['user_name'] }}' :
                            '{{ $lang['anonymous'] }}').'</td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input type="text" name="email" id="email" maxlength="100"
                                       value="{{ $smarty['session']['email'] }}" class="inputBorder"/>
                            </td>
                        </tr>
                        <tr>
                            <td align="right">{{ $lang['comment_rank'] }}：</td>
                            <td>
                                <input name="comment_rank" type="radio" value="1" id="comment_rank1"/> <img
                                    src="../images/stars1.gif"/>
                                <input name="comment_rank" type="radio" value="2" id="comment_rank2"/> <img
                                    src="../images/stars2.gif"/>
                                <input name="comment_rank" type="radio" value="3" id="comment_rank3"/> <img
                                    src="../images/stars3.gif"/>
                                <input name="comment_rank" type="radio" value="4" id="comment_rank4"/> <img
                            ...
                            <td colspan="2">
                                <!-- 判断是否启用验证码@if($enabled_captcha)
                                    ...
                                    <div style="padding-left:15px; text-align:left; float:left;">
                                        {{ $lang['comment_captcha'] }}：<input type="text" name="captcha"
                                                                              class="inputBorder"
                                                                              style="width:50px; margin-left:5px;"/>
                                        <img src="captcha.php?{{ $rand }}" alt="captcha"
                                             onClick="this.src='captcha.php?'+Math.random()" class="captcha">
                                    </div>
                                @endif
                                <input name="" type="submit" value="" class="f_r"
                                       style="border:none; background:url(../images/commentsBnt.gif); width:75px; height:21px; margin-right:8px;">
                            </td>
                        </tr>
                    </table>
                </form>
...
<script type="text/javascript">
    //<![CDATA[
    @foreach($lang['cmt_lang'] as $item => $key)
    var {{ $key }} = "{{ $item }}";
